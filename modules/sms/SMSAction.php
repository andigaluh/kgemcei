...
class SMSAction {
	...
	public static function loadAll($objectiveId) {
		return SMS::$instance->loadActions($objectiveId);
	}

	public static function load($id) {
		$action = SMS::$instance->loadAction($id);
		return $action;
	}
	...
	public function save() {
		$id = SMS::$instance->saveAction($this);
		if ($id) $this->id = $id;
		return $id;
	}
	...
	public static function getSelector($url, $objectiveId, $unitId) {
		...
		$div->addChild(new Hypertext(SMSObjective::getSimpleInfoPanel($objectiveId, $unitId)));
		$div->addChild(new LineBreak());

		$div->addChild(new ContentDiv("", t_("Select Action"), "SMSSubtitle"));
		...
		foreach (self::loadAll($objectiveId) as $action) {
			$table->rows[] = new DataRow(array($action->text, $action->weight, t_("From: ").DateUtil::unixToUIFull($action->begin)."&nbsp;&nbsp;".t_("To: ").DateUtil::unixToUIFull($action->end), SMSUnit::load($action->unitId)->name), "", "document.location='".$url."&obj_id={$objectiveId}&act_id={$action->id}';");
		}
	...
	public static function getEditor($unitId) {
		...
		$_SESSION["SMSActionURL"] = $url;

		$objectiveId = $_REQUEST["obj_id"];
		if (!$objectiveId) {
			return SMSObjective::getSelector($_SESSION["SMSActionURL"], $unitId);
		}

		$id = $_REQUEST["id"];
		$action = $id ? self::load($id) : new SMSAction(null, $objectiveId, $unitId, "", "", "", "");
		...
		if ($_REQUEST["edit"]) {
			...
			if ($_REQUEST["save"]) {
				...
				if (!$errors) {
					...
					$action->end = $end;
					$ok = $action->save();
					if ($ok) redirect_($_SESSION["SMSActionURL"]."&obj_id={$objectiveId}");
			...
			$div->addChild(new ContentDiv("", t_("Edit Action"), "SMSSubtitle"));
			$form = new Form("", $_SESSION["SMSActionURL"]);
			$div->addChild($form);
			...
			$pair = new LabelPair();
			$form->addChild($pair);
			SMSObjective::getSimpleInfoPanel($objectiveId, $unitId, $pair);
			...
			$form->addChild(new JSButton("", t_("Cancel"), "document.location='".$_SESSION["SMSActionURL"]."&obj_id={$objectiveId}';"));
			if ($action->id) {
				$form->addChild(new Hypertext("&nbsp;&nbsp;&nbsp;"));
				$form->addChild(new JSButton("", t_("Delete"), "document.location='".$_SESSION["SMSActionURL"]."&obj_id={$objectiveId}&del=1&id={$action->id}';", t_("Delete action?")));
			}
		}

		else if ($_REQUEST["del"]) {
			SMS::$instance->delAction($id);
			redirect_($_SESSION["SMSActionURL"]);
		}
			...
			$table->columns[] = new DataColumn(t_("Unit"));
			foreach (self::loadAll($objectiveId) as $action) {
				$table->rows[] = new DataRow(array($action->text, $action->weight, t_("From: ").DateUtil::unixToUIFull($action->begin)."&nbsp;&nbsp;".t_("To: ").DateUtil::unixToUIFull($action->end), SMSUnit::load($action->unitId)->name), "", "document.location='".$_SESSION["SMSActionURL"]."&obj_id={$objectiveId}&edit=1&id={$action->id}';");
			}
			$div->addChild(new LineBreak());
			$div->addChild(new Hyperlink($_SESSION["SMSActionURL"]."&obj_id={$objectiveId}&edit=1", t_("Add new action")));
			$div->addChild(new Hypertext("&nbsp;&nbsp;&nbsp;"));
			$div->addChild(new Hyperlink($_SESSION["SMSActionURL"], t_("Select other objective")));
		}
