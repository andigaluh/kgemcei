...
class SMSPerspective {
	...
	public static function loadAll() {
		return SMS::$instance->loadPerspectives();
	}

	public static function loadById($id) {
		return SMS::$instance->loadPerspectiveById($id);
	}

	public static function loadByCode($code) {
		return SMS::$instance->loadPerspectiveByCode($code);
	}
	...
	public function save() {
		$id = SMS::$instance->savePerspective($this);
		if ($id) $this->id = $id;
		return $id;
	}
	...
	public static function getEditor($request, $editURL = "") {
		...
		$mode = $request["mode"];
		if ($mode == "edit") {
			$perspective = $id ? self::loadById($id) : new SMSPerspective(null, "", "");
			...
			$buttons->addChild(new JSButton("", t_("Save"), SMS::$instance->getPerspectiveEditorSaveJS($formId)));
			$buttons->addChild(new Hypertext("&nbsp;&nbsp;"));
			$buttons->addChild(new JSButton("", t_("Cancel"), SMS::$instance->getPerspectiveEditorCloseJS()));
			if ($perspective->id) {
				$buttons->addChild(new Hypertext("&nbsp;&nbsp;"));
				$buttons->addChild(new JSButton("", t_("Delete"), SMS::$instance->getPerspectiveEditorDelJS($perspective->id), t_("Delete perspective?")));
			}
		}
		else if ($mode == "del") {
			SMS::$instance->delPerspective($id);
			return "";
		}
		else {
			...
			foreach (self::loadAll() as $perspective) {
				$table->rows[] = new DataRow(array($perspective->code, $perspective->name), "", SMS::$instance->getPerspectiveEditorOpenJS("edit", $perspective->id));
			}
			$div->addChild(new LineBreak());
			$div->addChild(new JSButton("", t_("Add New Perspective"), SMS::$instance->getPerspectiveEditorOpenJS("edit", "")));
		}
...
//			$url .= $o[1]."&";
//		}
//		$_SESSION["SMSPerspectiveURL"] = $url;
