...
class _hris_class_SelectOrgAjax extends AjaxListener {
   ...
   function _hris_class_SelectOrgAjax($act_name) {
      ...
      $this->_include_file = XOCP_DOC_ROOT."/modules/sms/class/ajax_selectorg.php";
      $this->init();
      parent::init();
   }
   ...
   function app_setOrg($args) {
      $db=&Database::getInstance();
      $user_id = getUserID();
      $_SESSION["sms_org_id"] = $args[0];
      $_SESSION["sms_dashboard_org"] = $args[0];
   }
...
class TreeOrg {
   ...
   function renderNodeOrg($node_id,$depth=0) {
      ...
      $sql = "SELECT b.org_id,c.org_class_id FROM ".XOCP_PREFIX."employee_job a"
            ." LEFT JOIN ".XOCP_PREFIX."jobs b ON a.job_id = b.job_id"
            ." LEFT JOIN ".XOCP_PREFIX."orgs c ON b.org_id = c.org_id"
            ." WHERE employee_id = '$person_id'";
      $result = $db->query($sql);
      list($person_org_id,$person_org_class_id)=$db->fetchRow($result);

      $sql = "SELECT a.org_id FROM hris_jobs a LEFT JOIN hris_employee_job b ON a.job_id = b.job_id WHERE b.employee_id = '$person_id'";
      $result = $db->query($sql);
      list($org_idy)=$db->fetchRow($result);

      if ($person_org_class_id == 2) {
         if ($this->item[$node_id]->parent == $org_idy) {
            $onclick = "onclick='_org_select_org(\"".$this->item[$node_id]->id."\",this,event);'";
         }else{
            $onclick = "";
         }
      }elseif ($person_org_class_id == 3) {
         if ($this->item[$node_id]->parent == $org_idy) {
            $onclick = "onclick='_org_select_org(\"".$this->item[$node_id]->id."\",this,event);'";
         }else{
            $onclick = "";
         }
      }elseif ($person_org_class_id == 4) {
         if ($this->item[$node_id]->parent == $org_idy) {
            $onclick = "onclick='_org_select_org(\"".$this->item[$node_id]->id."\",this,event);'";
         }else{
            $onclick = "";
