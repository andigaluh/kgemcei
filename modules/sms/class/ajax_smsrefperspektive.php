...
class _sms_class_SMSRefPerspektiveAjax extends AjaxListener {
   ...
   function app_newRefPerspektive($args) {
      ...
      $sql = "INSERT INTO sms_ref_perspektive (id,session,code,title,create_user_id,create_date)"
           . " VALUES('$id',$psid,'$code_refperspektive','$title_refperspektive','$create_user_id','$create_date')";
      $db->query($sql);
   ...
   function app_saveRefPerspektive($args) {
	 ...
	 if($title_refperspektive=="") {
      ...
      if($id=="new") {
         ...
         $id = $idx+1;
         $create_user_id = getUserID();
         $sql = "INSERT INTO sms_ref_perspektive (id,session,code,title,create_user_id)"
              . " VALUES('$id',$psid,'$code_refperspektive','$title_refperspektive','$create_user_id')";
         $db->query($sql);
      } else {
         $sql = "UPDATE sms_ref_perspektive SET session = '$psid', title = '$title_refperspektive',"
