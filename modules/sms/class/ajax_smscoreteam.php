...
if ( !defined('SMSCORETEAMAJAX_DEFINED') ) {
   define('SMSCORETEAMAJAX_DEFINED', TRUE);

require_once(XOCP_DOC_ROOT."/class/xocpajaxlistener.php");

class _smscoreteamAjax extends AjaxListener {
   
   function __construct($act_name) {
      $this->_act_name = $act_name;
      $this->_include_file = XOCP_DOC_ROOT."/modules/sms/class/ajax_smscoreteam.php";
      $this->init();
      parent::init();
   }
   ...
   function app_deleteRecord($args) {
      ...
      $id = $args[0];
      $sql = "DELETE FROM sms_core_team WHERE id = '$id'";
      $db->query($sql);
   
      $sql = "SELECT id,name,email,smtp_location FROM sms_core_team ORDER BY name"; /// the query
   ...
   function editor($id,$prefix) {
      ...
      $sql = "SELECT name,email,smtp_location FROM sms_core_team WHERE id = '$id'";
      $result = $db->query($sql);
   ...
   function app_addRecord($args) {
      ...
      $sql = "INSERT INTO sms_core_team(name,email,smtp_location) VALUES ('$name','$email','$smtp_location')";
      $db->query($sql);
   ...
   $db=&Database::getInstance();
	 ...
	 $sql = "SELECT person_nm,email,person_id FROM hris_persons WHERE person_id = $id";
	 $result = $db->query($sql);
	 list($person_nm,$email,$personid)=$db->fetchRow($result);
	 ...
	 //print_r($args); exit();
	  ...
	  $sql = "INSERT INTO sms_core_team(name,email,smtp_location,person_id) VALUES ('$name','$email','$smtp_location','$personid')";
      $db->query($sql);
      
      /// testf list all record
      $sql = "SELECT id,name,email,smtp_location,person_id FROM sms_core_team ORDER BY name"; /// the query
      $result = $db->query($sql); /// run the query, put in $result
      $tbldata = "<table class='xxlist'>"
			   . "<colgroup><col width='40'/><col width='200'/><col width='300'/><col width='300'/></colgroup>"
      ...
      if($db->getRowsNum($result)>0) {
         while(list($id,$name,$email,$smtp_location,$personid)=$db->fetchRow($result)) {
            ...
            $smtp_location = htmlentities($smtp_location,ENT_QUOTES);
			$numloop = $numloop +1;
	  		$tbldata .= "<tr><td style='text-align: center;'>$numloop</td><td><span class='xlnk' onclick='edit_record(\"$id\",this,event);'>$name</span></td><td>$email</td><td>$personid</td></tr>";
         }
      }
      $tbldata .= "</tbody></table>";
