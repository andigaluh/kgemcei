...
class PMSProgress {
	...
	public static function loadAll($personId) {
		return PMS::$instance->loadProgresses($personId);
	}

	public static function load($id) {
		return PMS::$instance->loadProgress($id);
	}
	...
	public function saveTarget() {
		$id = PMS::$instance->saveProgressTarget($this);
		if ($id) $this->id = $id;
		return $id;
	}

	public function saveActual() {
		$id = PMS::$instance->saveProgressActual($this);
		if ($id) $this->id = $id;
		return $id;
	}
	...
	public static function getEditor($unitId, $personId) {
		...
		$_SESSION["PMSProgressURL"] = $url;

		$id = $_REQUEST["id"];
		$progress = $id ? self::load($id) : new PMSProgress(null, "", "", $personId, "", "", "", "", "");
		$objectiveId = PMSAction::load($progress->actionId)->objectiveId;

		// edit objective
		if ($_REQUEST["edit"]) {
			if (!$objectiveId) $objectiveId = $_REQUEST["obj_id"];
			if (!$objectiveId) {
				return PMSObjective::getSelector($_SESSION["PMSProgressURL"]."&edit=1", $unitId);
			}
			...
			if ($_REQUEST["save"]) {
				...
				if (!$errors) {
					...
					if ($ok) redirect_($_SESSION["PMSProgressURL"]);
			...
			$div->addChild(new ContentDiv("", t_("Edit Progress Target"), "PMSSubtitle"));
			$form = new Form("", $_SESSION["PMSProgressURL"]);
			$div->addChild($form);
			...
			$pair = new LabelPair();
			$form->addChild($pair);
			PMSPerson::getInfoPanel($personId, $unitId, $pair);
			PMSObjective::getSimpleInfoPanel($objectiveId, $unitId, $pair, false);
			$as = array("" => "");
			foreach (PMSAction::loadAll($objectiveId) as $a) {
				$as[$a->id] = $a->text;
			}
			$pair->addChild("Action", new DropDown("ProgressAction", "ProgressAction", $as, $actionId));
			$is = array("" => "");
			foreach (PMSIndicator::loadAll($objectiveId) as $i) {
				$is[$i->id] = $i->text;
			}
			...
			$form->addChild(new JSButton("", t_("Cancel"), "document.location='".$_SESSION["PMSProgressURL"]."';"));
			if ($progress->id) {
				$form->addChild(new Hypertext("&nbsp;&nbsp;&nbsp;"));
				$form->addChild(new JSButton("", t_("Delete"), "document.location='".$_SESSION["PMSProgressURL"]."&del=1&id={$progress->id}';", t_("Delete progress?")));
			}
		}
		...
		// enter actual progress
			...
			$div->addChild(new ContentDiv("", t_("Edit Progress Actual"), "PMSSubtitle"));
			$form = new Form("", $_SESSION["PMSProgressURL"]);
			$div->addChild($form);
			$form->addChild(new HiddenInput("", "id", $id));
			...
			PMSPerson::getInfoPanel($personId, $unitId, $pair);
			PMSObjective::getSimpleInfoPanel($objectiveId, $unitId, $pair, false);
			$pair->addChild(t_("Action"), new PlainText(PMSAction::load($progress->actionId)->text));
			$pair->addChild(t_("Indicator"), new PlainText(PMSIndicator::load($progress->indicatorId)->text));
			...
			$form->addChild(new JSButton("", t_("Cancel"), "document.location='".$_SESSION["PMSProgressURL"]."';"));
			$form->addChild(new Hypertext("&nbsp;&nbsp;&nbsp;"));
			$form->addChild(new JSButton("", t_("Delete"), "document.location='".$_SESSION["PMSProgressURL"]."&actual=1&del=1&id={$progress->id}';", t_("Delete progress actual?")));
			$form->addChild(new Hypertext("&nbsp;&nbsp;&nbsp;"));
			$form->addChild(new JSButton("", t_("Edit Target"), "document.location='".$_SESSION["PMSProgressURL"]."&edit=1&id={$progress->id}';"));
		}

		else {
			$div->addChild(new Hypertext(PMSPerson::getInfoPanel($personId, $unitId)));
			$div->addChild(new LineBreak());

			$div->addChild(new ContentDiv("", t_("Progresses"), "PMSSubtitle"));
			...
			foreach (self::loadAll($personId) as $progress) {
				$table->rows[] = new DataRow(array($progress->text, $progress->weight, t_("From: ").DateUtil::unixToUIFull($progress->targetBegin)."&nbsp;&nbsp;".t_("To: ").DateUtil::unixToUIFull($progress->targetEnd), $progress->targetValue, $progress->actualValue, $progress->actualValue ? ($progress->actualEnd > $progress->targetEnd ? t_("Late") : t_("Done")) : (time() > $progress->targetEnd ? t_("Overdue") : t_("Undone"))), "", "document.location='".$_SESSION["PMSProgressURL"]."&actual=1&id={$progress->id}';");
			}
			$div->addChild(new LineBreak());
			$div->addChild(new Hyperlink($_SESSION["PMSProgressURL"]."&edit=1", t_("Add new progress")));
		}
