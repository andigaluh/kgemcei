...
class PMSIndicator {
	...
	public static function loadAll($objectiveId = null) {
		return PMS::$instance->loadIndicators($objectiveId);
	}

	public static function load($id) {
		$indicator = PMS::$instance->loadIndicator($id);
		return $indicator;
	}
	...
	public function save() {
		$id = PMS::$instance->saveIndicator($this);
		if ($id) $this->id = $id;
		return $id;
	}
	...
	public static function getEditor($unitId, $request = array(), $editURL = "") {
		...
		if ($mode == "edit") {
			$id = $request["id"];
			$indicator = $id ? self::load($id) : new PMSIndicator(null, "", "", "", "");
			...
			$buttons->addChild(new JSButton("", t_("Save"), PMS::$instance->getIndicatorEditorSaveJS($formId)));
			$buttons->addChild(new Hypertext("&nbsp;&nbsp;"));
			$buttons->addChild(new JSButton("", t_("Cancel"), PMS::$instance->getIndicatorEditorCloseJS()));
			if ($indicator->id) {
				$buttons->addChild(new Hypertext("&nbsp;&nbsp;"));
				$buttons->addChild(new JSButton("", t_("Delete"), PMS::$instance->getIndicatorEditorDelJS($indicator->id), t_("Delete indicator?")));
			}
		}
		else if ($mode == "del") {
			PMS::$instance->delIndicator($id);
			return "";
		}
		else {
			...
			$table->columns[] = new DataColumn(t_("Target Value"), "PMSValue", "PMSValue");
			foreach (self::loadAll($objectiveId) as $indicator) {
				$table->rows[] = new DataRow(array($indicator->text, $indicator->metric, $indicator->targetValue), "", PMS::$instance->getIndicatorEditorOpenJS("edit", $indicator->id, $objectiveId));
			}
			$div->addChild(new LineBreak());
			$div->addChild(new JSButton("", t_("Add New KPI"), PMS::$instance->getIndicatorEditorOpenJS("edit", "", $objectiveId)));
		}
