...
class _sms_class_MyApAjax extends AjaxListener {
   ...
   function init() {
      $this->registerAction($this->_act_name,"app_browseOrgs","app_selectOrg",
                                             ...
                                             "app_saveActionPlan","app_saveActionPlan_functional","app_setCurrentStatusDate","app_saveCurrentTargetAchievement",
                                             "app_createSnapshot","app_loadSnapshotHistory","app_selectEmployee",
                                             "app_editActionPlan","app_editActionPlanFunctional","app_newActionPlan","app_deleteActionPlan","app_deleteActionPlan_functional",
                                             ...
                                             "app_checkDeployObjectives","app_addnewap","app_saveNewAction","app_saveObjTargetText","app_submit","app_approval","app_setSessions","app_setSessions_functional");
   }
   ...
   //app_saveObjTargetText By FACHMY
	...
	//FUNCTIONAL
	  elseif ($no == 6) {
	  $sql = "UPDATE sms_pic_action_functional SET current_achievement = '$val' WHERE id = '$idap' AND psid = '$psid' AND employee_id = '$person_id' AND sms_objective_id = '$objid' AND actionplan_id = '$id_ksf' AND month_id = '$month_id' ";
       $db->query($sql); }
	
	  elseif ($no == 7) {
	  $sql = "UPDATE sms_pic_action_functional SET target_achievement = '$val' WHERE id = '$idap' AND psid = '$psid' AND employee_id = '$person_id' AND sms_objective_id = '$objid' AND actionplan_id = '$id_ksf' AND month_id = '$month_id' ";
       $db->query($sql); }
	
	  elseif ($no == 8) {
	  $sql = "UPDATE sms_pic_action_functional SET target_text = '$val' WHERE id = '$idap' AND psid = '$psid' AND employee_id = '$person_id' AND sms_objective_id = '$objid' AND actionplan_id = '$id_ksf' AND month_id = '$month_id' ";
       $db->query($sql); }
	
	  elseif ($no == 9) {
	  $sql = "UPDATE sms_pic_action_functional SET notes = '$val' WHERE id = '$idap' AND psid = '$psid' AND employee_id = '$person_id' AND sms_objective_id = '$objid' AND actionplan_id = '$id_ksf' AND month_id = '$month_id' ";
       $db->query($sql); 	}
  } 
...
/* 				$sqldel = "DELETE FROM sms_pic_action WHERE id = '$idap' AND actionplan_id = '$actionplan_id' AND employee_id = '$person_id' AND psid = '$psid' AND  sms_objective_id = '$sms_objective_id'";
      ...
      $result = $db->query($sql);
		...
		."<div style='padding:5px;' id='frmap'>"
          ...
          . "</td></tr>"
              . "<tr><td>Timeline : </td><td><select id='repeat_type' name='repeat_type' onchange='chgselrepeat(this,event);'>"
           ...
           . "</select></td></tr>"
           . "<tr><td>Month : </td><td><div id='month_range' style='".($repeat_type!=2?"":"display:none;")."'>"
...
//      return $ret;
   ...
   //SETSESSION
    function app_setSessions($args) {
      ...
      return array( $sesidap);
	   ...
	   //SETSESSION
    function app_setSessions_functional($args) {
	  ...
	  $_SESSION["psid_temps"] = $psid;
	  $_SESSION["person_id_temps"] = $person_id;
	  $_SESSION["objid_temps"] = $objids;
	  $_SESSION["sms_kpi_id_temps"] = $sms_kpi_id;
	  $_SESSION["month_id_ksf_temps"] = $month_id;
   ...
   function app_deleteActionPlan($args) {
      ...
      function app_deleteActionPlan_functional($args) {
      require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
      $db=&Database::getInstance();
      $user_id = getUserID();
      $idap = $args[0];
      ...
      $sql = "DELETE FROM sms_pic_action_functional WHERE employee_id = '$person_id' AND sms_objective_id = '$sms_objective_id' AND actionplan_id = '$actionplan_id' AND psid = '$psid' AND id = '$idap'";
      $db->query($sql);
      return array( $idap,$psid,$person_id,$actionplan_id,$sms_objective_id,$sql);
   }
   ...
   function app_submit($args) {
		...
		$sql = "UPDATE sms_pic_action_functional SET report_approval_st = '$status', report_submit_dttm = '$proposedate' WHERE psid = '$psid' AND employee_id ='$employee_id' AND month_id = '$month_id'";
		$db->query($sql);	
    ...
    $emailto = $this->emailmci($emailto);
     ...
     $headers["To"]      = $namato;
     //$headers["Cc"] =  $emailcc;   
     $headers["Subject"] = "HRIS Action Plan: Need Your Approval  ";
     ...
     $body =  "<!DOCTYPE html PUBLIC '-//W3C//DTD XHTML 1.0 Transitional//EN' 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'><html xmlns='http://www.w3.org/1999/xhtml'>
              ...
              <br/>
              <br/>
              A JAM form has been submitted to you, please click this <a href='http://".$_SERVER['SERVER_NAME']."".XOCP_SERVER_SUBDIR."/index.php?XP_smsapapproval'>link</a> to check and process approval.
              <br/>
              <br/>
   ...
   function app_approval($args) {
		...
		$sqlf = "UPDATE sms_pic_action_functional SET report_approval_st = '$status', report_approval_dttm = '$proposedate',approval1_employee_id = '$employee_id_approval' WHERE psid = '$psid' AND employee_id ='$employee_id' AND month_id = '$month_id'";
		$db->query($sqlf);	
...
/* 		$sqljob = "SELECT j.job_id,j.upper_employee_id,j.gradeval FROM hris_employee_job j LEFT JOIN hris_users u ON (u.person_id = j.employee_id) WHERE u.user_id = ".getUserid()."";
   ...
   //EDIT ACTION PLAN FUNCTIONAL
      function app_editActionPlanFunctional($args) {
      require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
      $db=&Database::getInstance();
      $user_id = getUserID();
      $idap = $args[0];
      ...
      $sql = "SELECT actionplan_text,target_text,month_id,actionplan_group_id,repeat_type FROM sms_pic_action_functional"
           . " WHERE employee_id = '$person_id' AND actionplan_id = '$actionplan_id' AND id ='$idap'";
      $result = $db->query($sql);
      if($db->getRowsNum($result)>0) {
         list($actionplan_text,$target_text,$month_id,$actionplan_group_id,$repeat_type)=$db->fetchRow($result);
      ...
      switch($repeat_type) {
         case 1:
            $sql = "SELECT month_id FROM sms_pic_action_functional WHERE employee_id = '$person_id' AND actionplan_id = '$actionplan_id' AND id = '$idap' ORDER BY month_id ";
            $result = $db->query($sql);
            $mno = 0;
            if($db->getRowsNum($result)>0) {
               while(list($month_idx)=$db->fetchRow($result)) {
      ...
      $ret = "<div style='padding:5px;' id='frmaps'>"
           ...
           . "<input type='hidden' name='actionplan_id' value='$actionplan_id'/>"
           . "<input type='hidden' name='psid' value='$psid'/>"
           . "<input type='button' onclick='save_actionplan_functional();' value='"._SAVE."'/>&nbsp;"
           . "<input type='button' onclick='close_actionplan();' value='"._CANCEL."'/>"
           . ($actionplan_id=="new"?"":"&nbsp;&nbsp;<input type='button' onclick='delete_actionplan_functional(\"$idap\",\"$psid\",\"$person_id\",\"$actionplan_id\",\"$sms_objective_id\",this,event);' value='"._DELETE."'/>")
           . "</td></tr>"
           . "</tbody></table>"
           . "</div>";
        // idap,psid,person_id,actionplan_id,sms_objective_id,
   ...
   //SAVE ACTION PLAN FUNCTIONAL
   function app_saveActionPlan_functional($args) {
      $psid = $_SESSION["pms_psid"];
      require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
      $db=&Database::getInstance();
      $user_id = getUserID();
	  ...
	  if($repeat_type==0){
		...
		$selmonth = $vars["selmonth"];
		$sqldel = "DELETE FROM sms_pic_action_functional WHERE id = '$idap' AND actionplan_id = '$actionplan_id' AND employee_id = '$person_id' AND psid = '$psid' AND  sms_objective_id = '$sms_objective_id'";
		$db->query($sqldel);
		
		$sqladd = "INSERT INTO sms_pic_action_functional (id,psid,employee_id,actionplan_id,sms_objective_id,repeat_type,month_id,actionplan_text,target_text,created_dttm,created_user_id) "
					 . " VALUES ('$idap','$psid','$person_id','$actionplan_id','$sms_objective_id','$repeat_type','$selmonth','$actionplan_text','$target_text','$date','$user_id') ";
		$db->query($sqladd);
