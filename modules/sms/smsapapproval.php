...
if ( !defined('SMS_APAPPROVAL_DEFINED') ) {
   define('SMS_APAPPROVAL_DEFINED', TRUE);
...
class _sms_ApApproval extends XocpBlock {
   var $catchvar = _SMS_CATCH_VAR;
   var $blockID = _SMS_APAPPROVAL_BLOCK;
   var $width = "100%";
   var $language;
   var $display_title = TRUE;
   var $title = "SMS Action Plan Approval";
   var $display_comment = TRUE;
   var $data;
   
   function _sms_ApApproval($catch=NULL) { /* fungsi konstruktor wajib punya parameter $catch
   ...
   function requestList() {
      ...
      $ret = "<table style='' class='xxlist'>"
	   ...
	   $sqlorg = "SELECT e.org_id, e.org_nm, f.org_class_id FROM hris_employee a LEFT JOIN hris_persons b USING ( person_id )  LEFT JOIN hris_employee_job c USING ( employee_id ) LEFT JOIN hris_jobs d ON d.job_id = c.job_id LEFT JOIN hris_orgs e USING ( org_id )  LEFT JOIN hris_org_class f USING ( org_class_id ) WHERE a.status_cd =  'normal' AND a.person_id =  ".getpersonid()."";
	      $resultorg = $db->query($sqlorg);
      list($org_ids,$org_nms,$org_class_ids)=$db->fetchRow($resultorg);
...
/* 	   $sql = "SELECT a.employee_id,b.employee_ext_id,c.person_nm "
   ...
   function approval($employee_id,$jam_org_ind,$jam_org_id) {
     ...
     $ret .= "<table style='margin-top:10px;table-layout:fixed;' class='xxlist'>";
	 $ret .= "<colgroup>"
						. "<col width='90'>"
						. "<col width='125'>"
						. "<col width='80'>"
	...
	$ret .= "<thead>"
				...
				. "</tr>"
				. "<tr>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=1' class='applelink' >Jan</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=2' class='applelink'>Feb</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=3' class='applelink'>Mar</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=4' class='applelink'>Apr</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=5' class='applelink'>May</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=6' class='applelink'>Jun</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=7' class='applelink'>Jul</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=8' class='applelink'>Aug</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=9' class='applelink'>Sep</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=10' class='applelink'>Oct</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=11' class='applelink'>Nov</a></td>"
					. "<td style='border-right:1px solid #bbb;text-align:center;'><a href='".XOCP_SERVER_SUBDIR."/index.php?XP_smsmyapapp&employee_id=$employee_id&goto=x&month_id=12' class='applelink'>Dec</a></td>"
				. "</tr>"
			."</thead>";
	...
	list($person_id)=$db->fetchRow($resultperid);
		...
		WHERE a.psid = $psid
		AND c.id_pic =$employee_id AND a.status = 'normal'";
	...
	if($db->getRowsNum($resultobj)>0) {
		...
		$sqlksf = "SELECT a.id,a.ksf_code,a.ksf_title,a.ksf_lower_perform,a.ksf_need_improvement,a.ksf_target,a.ksf_req,a.ksf_far_req  FROM sms_objective_ksf a JOIN sms_objective_team b ON(a.id =  b.id_objective_ksf) WHERE b.id_pic = $employee_id AND a.id_objective = $objid ";
	...
	//ACTION PLAN ADD
	$sqlksfin = "SELECT a.id,a.ksf_code,a.ksf_title,a.ksf_lower_perform,a.ksf_need_improvement,a.ksf_target,a.ksf_req,a.ksf_far_req  FROM sms_objective_ksf a JOIN sms_objective_team b ON(a.id =  b.id_objective_ksf) WHERE b.id_pic = $employee_id AND a.id_objective = $objid";
		...
		while(list($id_ksf,$ksf_code,$ksf_title,$ksf_lowper,$ksf_needimp,$ksf_target,$ksf_req,$ksf_far_req)=$db->fetchRow($resultksfin)){
			...
			$ret .= "<tr><td>Action Plan: </td><td  colspan='15' style='text-align:left;' id='tdaptext_new}' ><b>$ksf_title (<span class='xlnk applelink' onclick='add_actionplan(\"new\",\"$employee_id\",\"$id_ksf\",\"$objid\",this,event);' id='spap_${id_ksf}_new'>Add Action Plan</span>)</b></td></tr>";
		
		
		//PIC Action Plan
				$sqlap = "SELECT  id,sms_objective_id,employee_id,actionplan_id,month_id,actionplan_text,target_text,final_result_text,current_achievement,current_kpi_achievement,target_achievement,final_achievement,final_kpi_achievement  FROM sms_pic_action WHERE psid = '$psid' AND employee_id = '$employee_id' AND sms_objective_id = '$objid' AND actionplan_id  = '$id_ksf' GROUP BY actionplan_text";
			$resultap = $db->query($sqlap);
			 while(list($idap,$sms_objective_id,$employee_id,$actionplan_id,$month_id,$actionplan_text,$target_text,$final_result_text,$current_achievement,$current_kpi_achievement,$target_achievement,$final_achievement,$final_kpi_achievement)=$db->fetchRow($resultap)){
			   ...
			   $ret .= "<tr><td colspan='3' id='tdaptext_${idap}' style='border-right:1px solid #bbb;vertical-align:top;overflow:hidden;padding:0px;'>";
				...
				//MONTHS
					  for($i=1;$i<=12;$i++) {
						$sqlmnt = "SELECT month_id,current_achievement,report_approval_st FROM sms_pic_action WHERE psid = '$psid' AND employee_id = '$employee_id' AND sms_objective_id = '$objid' AND actionplan_id  = '$id_ksf' AND month_id = '$i' AND id ='$idap' ";
	...
	while(list($objid,$objective_title,$objective_weight)=$db->fetchRow($resultobj)){
		...
		//ACTION PLAN ADD Functional
			...
			while(list($objids,$sms_kpi_id,$sms_kpi_text)=$db->fetchRow($resultkpi)){
					...
					$sqlksfinfun = "SELECT  sms_action_plan_id,sms_action_plan_text,sms_kpi_id FROM sms_action_plan WHERE sms_action_plan_pic_employee_id LIKE '%$employee2id%' AND sms_objective_id = $objids AND sms_kpi_id = '$sms_kpi_id'  ORDER BY sms_kpi_id";
					...
					//MONTHS
					  for($i=1;$i<=12;$i++) {
						$sqlmnt = "SELECT month_id,current_achievement,report_approval_st FROM sms_pic_action_functional WHERE psid = '$psid' AND employee_id = '$employee2id' AND sms_objective_id = '$objid' AND actionplan_id  = '$kpid' AND month_id = '$i' AND id ='$id_ap' ";
						$resultmnt = $db->query($sqlmnt);
						list($month_id,$current_achievement,$report_approval_st)=$db->fetchRow($resultmnt);
						if($month_id==$i ){
		...
		$sqlsumjam = "SELECT SUM(value) FROM sms_jam_strategic WHERE person_id = '$employee_id' AND id_session = '$psid' ";
...
/* 	$form = "<div style='width:900px;text-align:right;padding:10px;margin-top:10px;margin-bottom:100px;'>"
    ...
    function showOrg($showOpt=FALSE) {
     ...
     function report($employee_id,$force_month_id) {
      ...
      $result = $db->query($sql);
      list($job_nm,$job_abbr,$org_nm,$org_abbr,$nip,$employee_nm,$person_id)=$db->fetchRow($result);
      $ret = "<div><div style='border:1px solid #bbb;background-color:#eee;padding:5px;text-align:right;'>[<a href='".XOCP_SERVER_SUBDIR."/index.php'>Back to Action Plan</a>]</div></div>";
