...
class _pms_Monitor extends XocpBlock {
   ...
   function calcYTD($pms_objective_id) {
      ...
      $val = $this->_calcSO($pms_objective_id,TRUE,$current_radar_month);
   ...
   function _calcSO($pms_objective_id,$report_ytd=TRUE,$report_month_id=0) {
      ...
      $rchild = $db->query($sql);
      if($pms_objective_id==1912) _debuglog($sql);
      $has_local_sub = 0;
      $ttl_sub_weight = 0;
   ...
   function createMarker($pno,$a,$b,$value = 0,$cause_effect_value = NULL, $ach_cause_effect = "no") {
      ...
      $marker .= "<svg:text x='$a' y='".($b+40)."' style='text-anchor:middle;fill:#bbb; font-size:11px;font-weight:normal;font-family:Arial;' filter='url(#drop_shadow_text${pno})'>".($selected_value!=-999?$value_txt:"n/a")."</svg:text>";
      $marker .= "</svg:g>";
      return $marker;
   }
   ...
   function pmsobjective() {
      ...
      if($db->getRowsNum($result)>0) {
         while(list($pms_objective_idx,$pms_perspective_idx,$pms_objective_text,$pms_perspective_code,$pms_objective_no,$show_dashboard,$pms_objective_weightx)=$db->fetchRow($result)) {
            ...
            $acharr[$pms_perspective_idx][$pms_objective_idx] = array($show_dashboard,$achYTD,$pms_objective_weightx);
            ...
            $gaugetip .= "<div id='gaugetip_${pms_objective_idx}'><div style='padding:5px;'>$pms_objective_idx <span style='color:#000;font-weight:bold;'>${pms_perspective_code}${pms_objective_no}</span> ".htmlentities($pms_objective_text)."</div><table style='font-size:0.9em;' class='xxlist'><tbody>";
            switch($org_class_id) {
               //// section
               case 4:
               case 5:
                  ...
                  if($db->getRowsNum($resultx1)>0) { ////// yes, has ap
                     ...
                     while(list($pms_actionplan_id,$actionplan_text)=$db->fetchRow($resultx1)) {
                        ...
                        if($db->getRowsNum($rsh)>0) {
                           while(list($pms_share_weightxxx,$pms_actionplan_pic_employee_idxxx)=$db->fetchRow($rsh)) {
                              $pic_ach = $this->_calcPeopleAch($pms_actionplan_pic_employee_idxxx,$pms_objective_idx,TRUE,$current_radar_month);
                              ...
                              $gaugetip .= "<tr><td style='${bgcolor}vertical-align:top;'>".htmlentities($employee_nmx)."</td>";
               ...
               default:
                  ...
                  if($has_local_sub>0) {
                     ...
                     foreach($initiative_weight as $sub_pms_objective_id=>$sub_weight) {
                           ...
                           foreach($kpi_share_array as $pms_share_org_idx=>$pms_share_weightx) {
                              ...
                              if($db->getRowsNum($rorg)>0) {
                                 while(list($sub_pms_objective_idxxx,$sub_pms_objective_textxxx,$sub_pms_org_idxxx,$sub_org_nmxxx,$pms_perspective_codexxx,$pms_objective_noxxx)=$db->fetchRow($rorg)) {
                                    ...
                                    $ach_sub_so = $this->_calcSO($sub_pms_objective_idxxx,TRUE,$current_radar_month);
                                    ...
                                    $gaugetip .= "<tr><td style='${bgcolor}vertical-align:top;'>"
                                               . "<span class='xlnk' onclick='drill_down_org(\"$sub_pms_org_idxxx\",\"$sub_pms_objective_idxxx\",this,event);'>".htmlentities($sub_org_nmxxx)."</span>"
                                               . "</td>";
                        ...
                        $gaugetip .= "<tr>"
                                   . "<td style='font-weight:bold;background-color:#eee;'>Subtotal Result</td>"
                                   . "<td style='font-weight:bold;background-color:#eee;text-align:right;vertical-align:top;' colspan='4'>".toMoney($ttl_ach_sub_so_weight)." / ".toMoney($ttl_share)." = ".toMoney($ttl_ach_sub_so_weight/$ttl_share)."</td></tr>";
                        
                        $ttl_initiative += $ttl_ach_sub_so_weight/$ttl_share;
                        $ttl_initiative_weight += ($ttl_ach_sub_so_weight/$ttl_share)*$sub_weight;
                        
                        $arr_ttl[] = array($ttl_ach_sub_so_weight/$ttl_share,$sub_weight);
                     ...
                     $subach_txt = "";
                     $subweight_txt = "";
                     foreach($arr_ttl as $v) {
                        list($a,$b)=$v;
                        $subach_txt .= toMoney($a)."x".toMoney($b)." + ";
                        $subweight_txt .= toMoney($b)." + ";
                        $ttl_subweight += $b;
                        $ttl_subach += ($a*$b);
                     }
                     $subach_txt = substr($subach_txt,0,-3);
                     $subweight_txt = substr($subweight_txt,0,-3);
                     $gaugetip .= "<tr>"
                                . "<td style='font-weight:bold;background-color:#ddd;color:#000;'>Total Result</td>"
                                . "<td style='font-weight:bold;background-color:#ddd;color:#000;text-align:right;vertical-align:top;' colspan='4'> ($subach_txt) / ($subweight_txt) = ".toMoney($ttl_subach/$ttl_subweight)."</td></tr>";
                     ...
                     foreach($kpi_share_array as $pms_share_org_idx=>$pms_share_weightx) {
                        ...
                        if($db->getRowsNum($rorg)>0) {
                           while(list($sub_pms_objective_id,$sub_pms_objective_text,$sub_pms_org_id,$sub_org_nm,$pms_perspective_code,$pms_objective_no)=$db->fetchRow($rorg)) {
                              ...
                              $ach_sub_so = $this->_calcSO($sub_pms_objective_id,TRUE,$current_radar_month);
                              if($ach_sub_so!=-999) {
                                 $ach_sub_so_weight = ($pms_share_weightx*$ach_sub_so);
                                 $ttl_ach_sub_so_weight += $ach_sub_so_weight;
                              }
      ...
      foreach($acharr as $pms_perspective_id=>$v) {
         foreach($v as $pms_objective_idx=>$vv) {
            ...
            list($show_dashboardx,$achx,$weightx)=$vv;
         ...
         while(list($pms_perspective_code,$pms_perspective_id,$pms_perspective_name)=$db->fetchRow($result)) {
            ...
            if($cnt>0) {
               ...
               foreach($acharr[$pms_perspective_id] as $pms_objective_id=>$vvx) {
                  ...
                  $svg .= "<svg:rect x='".($cx[$no]-75)."' y='".$cy[$no]."' width='150' height='150' filter='url(#ig${pno})' onclick='dashboard_tooltip(\"$pms_objective_id\",this,evt);'/>";
                  $lw = 0;
