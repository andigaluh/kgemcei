...
class _pms_class_ObjectiveAjax extends AjaxListener {
   ...
   function init() {
      $this->registerAction($this->_act_name,"app_browseOrgs","app_selectOrg",
                                             ...
                                             "app_saveJAMTargetText",
                                             
                                             /// jam create
                                             "app_submitJAM","app_approval1JAM",
                                             "app_firstAssessorReturnJAM","app_approval2JAM","app_nextAssessorReturnJAM",
                                             
                                             /// jam report
                                             "app_saveJAMFinalResult","app_saveJAMKPIFinalResult","app_saveJAMAPFinalResult",
                                             "app_submitJAMReport","app_approval1JAMReport",
                                             "app_firstAssessorReturnJAMReport","app_approval2JAMReport","app_nextAssessorReturnJAMReport",
                                             "app_resetJAMAPFinalResult","app_resetJAMKPIFinalResult","app_resetJAMFinalResult",
                                             "app_undoNextApprovalJAMReport",
                                             ...
                                             "app_checkDeployObjectives","app_deploySingleKPI","app_renderActionPlanReport",
                                             "app_reportNextMonth");
   }
   
   function app_resetJAMFinalResult($args) {
   ...
   function app_resetJAMKPIFinalResult($args) {
      ...
      $sql = "UPDATE pms_jam SET kpi_final_result = '$kpi_final_result'"
           . " WHERE employee_id = '$employee_id' AND pms_objective_id = '$pms_objective_id'";
      $db->query($sql);
   ...
   function app_resetJAMAPFinalResult($args) {
      $db=&Database::getInstance();
      ...
      $sql = "UPDATE pms_jam SET ap_final_result = '$ap_final_result'"
           . " WHERE employee_id = '$employee_id' AND pms_objective_id = '$pms_objective_id'";
      $db->query($sql);
      
      return toMoney($ap_final_result);
   }
   
   function app_reportNextMonth($args) {
      $dir = $args[0];
      $employee_id = $args[1];
      $_SESSION["pms_month"] += $dir;
      if($_SESSION["pms_month"]<1) $_SESSION["pms_month"] = 1;
      if($_SESSION["pms_month"]>12) $_SESSION["pms_month"] = 12;
      return $this->app_renderActionPlanReport(array($employee_id));
   }
   
   function app_renderActionPlanReport($args) {
      ...
      $ret = "";
      $ret .= "<table style='width:850px;table-layout:fixed;' class='xxlist'>"
            . "<colgroup>"
            . "<col width='180'/>";
      $ret .= "<col width='50'/>";
      $ret .= "<col width='50'/>";
      ...
      $ret .= "<thead>"
            . "<tr>"
               . "<td style='border-right:1px solid #bbb;text-align:center;' rowspan='2'>Action Plan</td>"
               . "<td colspan='2'  style='border-right:1px solid #bbb;text-align:center;'>Achievement</td>"
               . "<td style='border-right:1px solid #bbb;text-align:center;' colspan='3'>PICA</td>"
               . "<td style='text-align:center;border-right:1px solid #bbb;' rowspan='2'>Target</td>"
               . "<td style='text-align:center;border-right:0px solid #bbb;' rowspan='2'>Report<br/>Status</td>"
            . "</tr>"
            . "<tr>";
      $ret .= "<td style='text-align:center;border-right:1px solid #bbb;'>AP</td>";
      $ret .= "<td style='text-align:center;border-right:1px solid #bbb;'>KPI</td>";
      ...
      $ret .= "<td style='text-align:center;border-right:1px solid #bbb;'>Next<br/>Schedule</td>";
      $ret .= "</tr>"
            . "</thead>";
      ...
      if($db->getRowsNum($result)>0) {
         ...
         while(list($pms_objective_no,$pms_perspective_code,$pms_org_id,$pms_objective_id,$pms_objective_text,$objective_weight,
            ...
            $sql = "SELECT repeat_type,actionplan_id,actionplan_text,current_achievement,current_kpi_achievement,"
                 . "actionplan_group_id,root_cause,improvement_text,pica_id,report_approval_st,target_text,is_pica"
                 . " FROM pms_pic_action"
                 . " WHERE pms_objective_id = '$pms_objective_id'"
                 . " AND employee_id = '$employee_id'"
                 . " AND psid = '$psid'"
                 . " AND month_id = '$current_month'";
            $rmon = $db->query($sql);
            if($db->getRowsNum($rmon)>0) {
               $ret .= "<tr><td colspan='8' style='padding:5px;background-color:#555;color:#fff;'><span style='font-weight:bold;'>${pms_perspective_code}${pms_objective_no} $pms_objective_text</span></td></tr>";
               while(list($repeat_type,$actionplan_id,$actionplan_text,$current_achievement,$current_kpi_achievement,$actionplan_group_id,
                          $pica_root_cause,$pica_improvement_text,$pica_id,$report_approval_st,$target_text,$is_pica)=$db->fetchRow($rmon)) {
                  if($repeat_type>0) continue;
                  if($actionplan_group_id>0) {
                     $sql = "SELECT actionplan_text"
                          . " FROM pms_pic_action"
                          . " WHERE pms_objective_id = '$pms_objective_id'"
                          . " AND employee_id = '$employee_id'"
                          . " AND psid = '$psid'"
                     ...
                     $rapgroup = $db->query($sql);
                     if($db->getRowsNum($rapgroup)>0) {
                        list($actionplan_text)=$db->fetchRow($rapgroup);
                     } else {
                        //// should delete it here ...
                        continue;
                     }
                  }
                  ...
                  //// base100 mod start
                  if($is_pica==1) {
                     list($current_achievement,$current_kpi_achievement)=_pms_getActionPlanAchievement($pms_objective_id,$employee_id,$actionplan_id,TRUE);
                  }
                  //// base100 mod end
                  
                  
                  $ret .= "<tr class='$xclass'><td style='border-right:1px solid #bbb;'>$actionplan_text</td>"
                        . "<td style='border-right:1px solid #bbb;text-align:center;'><span class='xlnk' onclick='edit_report_ap(\"$pms_objective_id\",\"$actionplan_id\",\"$no\",this,event);'>".toMoney($current_achievement)."</span></td>"
                        . "<td style='border-right:1px solid #bbb;text-align:center;'><span class='xlnk' onclick='edit_report_ap(\"$pms_objective_id\",\"$actionplan_id\",\"$no\",this,event);'>".toMoney($current_kpi_achievement)."</span></td>"
                        . "<td style='border-right:1px solid #bbb;'>".htmlentities($pica_root_cause,ENT_QUOTES)."</td>"
                        . "<td style='border-right:1px solid #bbb;'>".htmlentities($pica_improvement_text,ENT_QUOTES)."</td>"
                        . "<td style='border-right:1px solid #bbb;'>$pica_month_text</td>"
                        . "<td style='border-right:1px solid #bbb;'>".htmlentities($target_text,ENT_QUOTES)."</td>"
                        . "<td style='text-align:center;".($report_approval_st!="final"?"background-color:#ddf;":"")."'>$report_approval_st</td>"
                        . "</tr>";
               }
            }
      ...
      $submit_employee = (($apreport_need_submission>0)&&$self_job_id==$job_id?"<input type='button' value='Submit Report' onclick='confirm_submit_apreport(this,event);'/>":"")
      ...
      $submit_superior = ($apreport_need_submission>0&&!(($apreport_need_approval>0)&&$self_job_id==$first_assessor_job_id)?"-":"")
      ...
      return array($xocp_vars["month_year"][$current_month],$ret,$submit_employee,$submit_superior);
   ...
   function app_submitActionPlanReport($args) {
      ...
      if($do_email>0) {
         $this->emailApproval($self_employee_id,$first_assessor_employee_id,"report");
      }
   ...
   function app_savePICA($args) {
      ...
      $sql = "SELECT month_id,actionplan_text,final_achievement,final_kpi_achievement,actionplan_group_id FROM pms_pic_action"
           . " WHERE psid = '$psid'"
           . " AND employee_id = '$employee_id'"
           . " AND pms_objective_id = '$src_pms_objective_id'"
           . " AND actionplan_id = '$src_actionplan_id'";
      $result = $db->query($sql);
      list($month_id,$actionplan_text,$final_achievement,$final_kpi_achievement,$actionplan_group_id)=$db->fetchRow($result);
      if($actionplan_group_id>0) {   /// in group
         ...
         if($cnt>0) {
            $xx_ap_achievement = $ttl_ap_achievement/$cnt;
            $xx_kpi_achievement = $ttl_kpi_achievement/$cnt;
         } else {
            $xx_ap_achievement = $xx_kpi_achievement = 0;
         }
         
         if($final_achievement==="") $final_achievement = $xx_ap_achievement;
         if($final_kpi_achievement==="") $final_kpi_achievement = $xx_kpi_achievement;
         
      } else { /// carry over
         $ttl_ap_achievement = 0;
         $ttl_kpi_achievement = 0;
         $_SESSION["pica_actionplan_id"] = array();
         $_SESSION["pica_actionplan_id"][$src_actionplan_id] = 1;
         $pica_root = $this->getpicaroot($src_pms_objective_id,$employee_id,$src_actionplan_id);
         foreach($_SESSION["pica_actionplan_id"] as $pica_actionplan_id=>$xz) {
            ...
            if($db->getRowsNum($rac)>0) {
               while(list($x_month_id,$x_current_achievement,$x_current_kpi_achievement,$x_target_achievement,$x_pica_id,$x_is_pica)=$db->fetchRow($rac)) {
                  ...
                  $ttl_kpi_achievement += $x_current_kpi_achievement;
                  $vv = "x_ap_${x_month_id}";
                  list($_current_achievement,$_current_kpi_achievement) = _pms_getActionPlanAchievement($src_pms_objective_id,$employee_id,$pica_actionplan_id,TRUE);
                  $$vv = toMoneyShort($_current_achievement)."%";
                  $ww = "x_kpi_${x_month_id}";
                  $$ww = toMoneyShort($_current_kpi_achievement)."%";
               }
            }
            
            if($final_achievement==="") {
               $final_achievement = $ttl_ap_achievement;
            }
            if($final_kpi_achievement==="") {
               $final_kpi_achievement = $ttl_kpi_achievement;
            }
      ...
      $sql = "SELECT a.pms_objective_text,a.pms_objective_no,b.pms_perspective_code"
           . " FROM pms_objective a"
           . " LEFT JOIN pms_perspective b USING(pms_perspective_id)"
           . " WHERE a.pms_objective_id = '$src_pms_objective_id'";
      $result = $db->query($sql);
      list($pms_objective_text,$pms_objective_no,$pms_perspective_code)=$db->fetchRow($result);
      
      
      $ret = "<div style='padding:10px;color:blue;padding-top:5px;'>Please fulfill year to date achievement value below:</div>"
      ...
      $ret .= "<tr><td>Action Plan Year to Date Achievement:</td><td><input type='text' style='width:50px;' id='final_achievement' value='$final_achievement'/> %</td></tr>";
      $ret .= "<tr><td>KPI Year to Date Achievement:</td><td><input type='text' style='width:50px;' id='final_kpi_achievement' value='$final_kpi_achievement'/> %</td></tr>";
   ...
   function app_setAchievement($args) {
      ...
      //// base100 mod start
      ////// back translate from base 100
      if($is_pica==1) {
         list($prev_achievement,$prev_kpi_achievement)=_pms_getActionPlanAchievement($pms_objective_id,$employee_id,$actionplan_id,FALSE);
         $achievement = _bctrim(bcsub($achievement,$prev_achievement));
         $achievement_kpi = _bctrim(bcsub($achievement_kpi,$prev_kpi_achievement));
      }
      //// base100 mod end
      ...
      if($gap_ap<-9||$gap_kpi<-9) { /// pica
         ...
         ////// back translate from base 100
         if($is_pica==1) {
            list($achievement,$achievement_kpi)=_pms_getActionPlanAchievement($pms_objective_id,$employee_id,$actionplan_id,TRUE);
         }
         //// base100 mod end
         ...
         if($actionplan_group_id>0) {
            ...
            if($is_pica==1) {
               ...
               foreach($_SESSION["pica_actionplan_id"] as $pica_actionplan_id=>$xz) {
                  ...
                  if($db->getRowsNum($rac)>0) {
                     while(list($x_month_id,$x_current_achievement,$x_current_kpi_achievement,$x_target_achievement,$x_pica_id,$x_is_pica)=$db->fetchRow($rac)) {
                        list($_current_achievement,$_current_kpi_achievement) = _pms_getActionPlanAchievement($pms_objective_id,$employee_id,$pica_actionplan_id,TRUE);
                        $vv = "x_ap_${x_month_id}";
                        $$vv = "$_current_achievement%";
                        $ww = "x_kpi_${x_month_id}";
                        $$ww = "$_current_kpi_achievement%";
                        $ttl_ap_achievement += $x_current_achievement;
                        $ttl_kpi_achievement += $x_current_kpi_achievement;
                     }
                  }
   ...
   function app_submitActionPlan($args) {
      ...
      if($do_email>0) {
         $this->emailApproval($self_employee_id,$first_assessor_employee_id,"plan");
      }
   }
   ...
   function getActionPlanReportUpdate($pms_objective_id,$employee_id,$report_month=0) {
      ...
      foreach($aptextarr as $no=>$v) {
         ...
         $vardvtg .= htmlentities($target_text,ENT_QUOTES);
   ...
   function app_saveActionPlan($args) {
      ...
      $sql = "UPDATE pms_pic_action SET "
           ...
           . "repeat_type = '$repeat_type', approval_st = 'new'"
           . " WHERE employee_id = '$employee_id' AND pms_objective_id = '$pms_objective_id' AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
      ...
      $sql = "UPDATE pms_pic_action SET "
           . "actionplan_text = '".addslashes($ap_text)."',"
           . "target_text = '".addslashes($tg_text)."', approval_st = 'new'"
   ...
   //// JAM FINAL REPORTING
   function app_nextAssessorReturnJAMReport($args) {
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $employee_id = $args[0];
      $jam_org_ind = $args[1];
      $jam_org_id = $args[2];
      $return_note = addslashes(urldecode($args[3]));
      $sql = "UPDATE pms_jam SET report_approval_st = 'return', report_return_note = '$return_note' WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $db->query($sql);
   }
   
   function app_undoNextApprovalJAMReport($args) {
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $employee_id = $args[0];
      $jam_org_ind = $args[1];
      $jam_org_id = $args[2];
      $sql = "UPDATE pms_jam SET report_approval_st = 'approval2', report_approval2_dttm = now() WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $db->query($sql);
   }
   
   function app_approval2JAMReport($args) {
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $employee_id = $args[0];
      $jam_org_ind = $args[1];
      $jam_org_id = $args[2];
      $sql = "UPDATE pms_jam SET report_approval_st = 'final', report_approval2_dttm = now() WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $db->query($sql);
      $sql = "INSERT INTO pms_jam_history SELECT NULL,now(),a.* FROM pms_jam a WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $db->query($sql);
   }
   
   function app_firstAssessorReturnJAMReport($args) {
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $employee_id = $args[0];
      $jam_org_ind = $args[1];
   ...
   function app_approval1JAMReport($args) {
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $employee_id = $args[0];
      $jam_org_ind = $args[1];
      $jam_org_id = $args[2];
      $sql = "UPDATE pms_jam SET report_approval_st = 'approval2', report_approval1_dttm = now() WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $db->query($sql);
   }
   
   function app_submitJAMReport($args) {
      $psid = $_SESSION["pms_psid"];
      require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
      $db=&Database::getInstance();
      $user_id = getUserID();
      ...
      $sql = "UPDATE pms_jam SET report_submit_dttm = now(),"
           . " report_approval_st = 'approval1',"
   ...
   function app_saveJAMKPIFinalResult($args) {
      global $norm_color;
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $user_id = getUserID();
      ...
      $sql = "SELECT objective_weight FROM pms_jam WHERE pms_objective_id = '$pms_objective_id' AND employee_id = '$employee_id'";
      $result = $db->query($sql);
      list($objective_weight)=$db->fetchRow($result);
      ...
      $sql = "UPDATE pms_jam SET kpi_final_result = '$val', final_modified_ind = '1' WHERE psid = '$psid' AND employee_id = '$employee_id' AND pms_objective_id = '$pms_objective_id'";
      $db->query($sql);
      
      $jam_org_ind = $_SESSION["pms_jam_org"];
      
      $sql = "SELECT sum(a.kpi_final_result),sum(a.ap_final_result),sum(a.reported_final_result)"
           . " FROM pms_jam a"
      ...
      $ttl = 0;
      if($db->getRowsNum($result)>0) {
         list($ttl_kpi,$ttl_ap,$ttl_ttl)=$db->fetchRow($result);
      } else {
         $ttl_kpi = $ttl_ap = $ttl_ttl = 0;
      }
      
      $norm_result = _bctrim(($val*100)/$objective_weight);
      $bgcolor = "#aaffaa";
      ksort($norm_color);
      ...
      return array(toMoney($val),$bgcolor);
   }
   
   function app_saveJAMAPFinalResult($args) {
      global $norm_color;
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $user_id = getUserID();
      ...
      $sql = "UPDATE pms_jam SET ap_final_result = '$val', final_modified_ind = '1' WHERE psid = '$psid' AND employee_id = '$employee_id' AND pms_objective_id = '$pms_objective_id'";
      $db->query($sql);
   ...
   function app_saveJAMFinalResult($args) {
      global $norm_color;
      $psid = $_SESSION["pms_psid"];
      $db=&Database::getInstance();
      $user_id = getUserID();
      ...
      $sql = "UPDATE pms_jam SET reported_final_result = '$val', final_modified_ind = '1' WHERE psid = '$psid' AND employee_id = '$employee_id' AND pms_objective_id = '$pms_objective_id'";
      $db->query($sql);
      ...
      $ttl = 0;
      if($db->getRowsNum($result)>0) {
         while(list($xval)=$db->fetchRow($result)) {
            $ttl = _bctrim(bcadd($ttl,$xval));
         }
      }
      ...
      _debuglog("$norm_result $bgcolor");
      
      return array(toMoney($val),toMoney($ttl)." %",$bgcolor,$ttl_bgcolor);
   }
   ...
   function emailApproval($employee_id,$approval_employee_id,$approval_type="report") {
      $db=&Database::getInstance();
      
      $sql = "SELECT b.person_nm,b.smtp_location,b.email FROM ".XOCP_PREFIX."employee a"
           . " LEFT JOIN ".XOCP_PREFIX."persons b USING(person_id)"
           . " WHERE a.employee_id = '$employee_id'";
      $result = $db->query($sql);
      list($employee_nm,$employee_smtp,$employee_email)=$db->fetchRow($result);
      
      $sql = "SELECT b.person_nm,b.smtp_location,b.email FROM ".XOCP_PREFIX."employee a"
           . " LEFT JOIN ".XOCP_PREFIX."persons b USING(person_id)"
           . " WHERE a.employee_id = '$approval_employee_id'";
      $result = $db->query($sql);
      list($approval_nm,$approval_smtp,$approval_email)=$db->fetchRow($result);
      
      list($user_mail,$domain_mail)=explode("@",$approval_email);
      $email = "${user_mail}@${approval_smtp}";
      $subject = "PMS - Action Plan ".($approval_type=="report"?"Report ":"")."Approval";
...
<title>PMS - Mitsubishi Chemical Indonesia</title>
</head>
<body>
...
You have Action Plan ".($approval_type=="report"?"Report ":"")."approval request from $employee_nm.

You can approve by login to system by opening this address http://146.67.1.47/hris using Mozilla Firefox.
...
";
                  ...
                  $approval_smtp = strtoupper($approval_smtp);
                  
                  switch($approval_smtp) {
                     case "MKMS01":
