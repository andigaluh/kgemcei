...
class _pms_class_ObjectiveAjax extends AjaxListener {
   ...
   function recalculate_kpi_weight($psid,$pms_org_id,$pms_objective_id) {
      ...
      $sql = "SELECT pms_kpi_id,pms_share_org_id,pms_share_weight FROM pms_kpi_share WHERE psid = '$psid' AND pms_objective_id = '$pms_objective_id' AND pms_org_id = '$pms_org_id'";
      $result = $db->query($sql);
      $ttl_kpi = 0;
      $arr_kpi = array();
      ...
      foreach($arr_kpi as $pms_kpi_id=>$v) {
         foreach($v as $pms_share_org_id=>$vv) {
            ...
            $sql = "UPDATE pms_kpi_share SET pms_share_weight = '$new_share_weight' WHERE psid = '$psid' AND pms_objective_id = '$pms_objective_id' AND pms_kpi_id = '$pms_kpi_id' AND pms_share_org_id = '$pms_share_org_id'";
            $db->query($sql);
         }
      }
   ...
   function insert_new_objective($psid,$pms_org_id,$pms_objective_text,$pms_perspective_id) {
      ...
      $sql = "INSERT INTO pms_objective (psid,pms_org_id,pms_objective_text,pms_perspective_id,pms_objective_no)"
           . " VALUES('$psid','$pms_org_id','$pms_objective_text','$pms_perspective_id','$new_pms_objective_no')";
   ...
   function calcTotalShared($pms_perspective_id=0) {
      ...
      return array(toMoney($total_shared),$arr_org,$arr_pers,(bccomp(number_format($ttlw,2,".",""),number_format($total_shared,2,".",""))));
   ...
   function app_calcRemainingShare($args) {
      ...
      $sql = "SELECT a.pms_share_weight,a.pms_share_org_id,a.pms_kpi_id FROM pms_kpi_share a"
      ...
      //// check link deploy
      $sql = "SELECT pms_objective_weight FROM pms_objective WHERE psid = '$psid' AND pms_parent_objective_id = '$pms_objective_id' AND pms_org_id = '$pms_share_org_id'";
      $result = $db->query($sql);
      $has_sub = 0;
      if($db->getRowsNum($result)>0) {
         while(list($sub_pms_objective_weight)=$db->fetchRow($result)) {
         
         }
         $info = "";
         $has_sub = 1;
      } else {
         $info = "<div style='text-align:center;font-size:0.9em;color:#ff0000;margin-top:5px;'>This KPI has not been deploy yet."
      ...
      $ret = array($remaining,
                   ...
                   $this_share_100,
                   $remaining_100,
                   $this->calcTotalShared($pms_perspective_id),$has_sub);
   ...
   function app_editActionPlan($args) {
      ...
      $sql = "SELECT actionplan_text,target_text,month_id,allow_carry_over,actionplan_group_id,repeat_type FROM pms_pic_action"
      ...
      switch($repeat_type) {
         case 1:
            $sql = "SELECT month_id FROM pms_pic_action WHERE employee_id = '$employee_id' AND pms_objective_id = '$pms_objective_id' AND actionplan_group_id = '$actionplan_id' ORDER BY month_id";
            $result = $db->query($sql);
            $mno = 0;
            if($db->getRowsNum($result)>0) {
   ...
   function app_saveSO($args) {
      ...
      $sql = "UPDATE pms_objective SET "
           ...
           . ($allow_add_objective[$current_org_class_id]==1?"pms_objective_weight = '$weight',":"")
      ...
      $ret = array($is_insert,
                   ...
                   $is_local_sub,
                   $prev_objective_id,             //// previous pms_objective_id (initiative) at the same parent
      ...
      $ret = "<div style='height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;'>"
           ...
           . "<div style='padding:5px;background-color:#f2f2f2;color:#555;'>"
              ...
              . "<div style='border:1px solid #999;background-color:#fff;padding:4px;' id='frmobjective'>"
                  ...
                  . "<tr><td>Weight</td><td>"
                     . "<input ".($allow_add_objective[$current_org_class_id]==1?"":($has_local_sub>0?"":"disabled='1'"))." onclick='_dsa(this);' onkeypress='kp_so_weight(this,event);' id='weight' name='weight' type='text' style='width:180px;' value='$pms_objective_weight_subcalc'/>"
                     . " %</td></tr>"
                  . "<tr><td>Time Frame</td><td>"
                  . "<span class='xlnk' id='sp_tm_start' onclick='_changedatetime(\"sp_tm_start\",\"h_tm_start\",\"date\",true,false);'>".sql2ind($tm_start,"date")."</span>"
                  . "<input type='hidden' name='h_tm_start' id='h_tm_start' value='$tm_start'/>"
