...
if ( !defined('HRIS_CLASS_AJAXJAMORGACCESS_DEFINED') ) {
   define('HRIS_CLASS_AJAXJAMORGACCESS_DEFINED', TRUE);

require_once(XOCP_DOC_ROOT."/class/xocpajaxlistener.php");

class _hris_class_JAMOrgAccessAjax extends AjaxListener {
   
   function __construct($act_name) {
      $this->_act_name = $act_name;
      $this->_include_file = XOCP_DOC_ROOT."/modules/pms/class/ajax_jamorgaccess.php";
      $this->init();
      parent::init();
   }
   ...
   function app_delAccess($args) {
      ...
      $sql = "UPDATE pms_jam_org_access SET status_cd = 'nullified', nullified_dttm = now(), nullified_user_id = '$user_id'"
           . " WHERE access_id = '$access_id'";
      $db->query($sql);
      
      $sql = "SELECT pms_org_id FROM pms_jam_org_access"
           . " WHERE access_id = '$access_id'";
      $result = $db->query($sql);
   ...
   function app_addAccess($args) {
      ...
      if($t=="person") {
         $employee_id = $args[3];
         $sql = "SELECT access_id FROM pms_jam_org_access"
         ...
         if($db->getRowsNum($result)==0) {
            ...
            $sql = "INSERT INTO pms_jam_org_access (psid,pms_org_id,employee_id,job_id,created_user_id)"
...
} /// HRIS_CLASS_AJAXJAMORGACCESS_DEFINED
?>