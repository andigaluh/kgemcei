...
if ( !defined('PMS_JAMREPORTAPPROVAL_DEFINED') ) {
   define('PMS_JAMREPORTAPPROVAL_DEFINED', TRUE);

include_once(XOCP_DOC_ROOT."/modules/hris/modconsts.php");
include_once(XOCP_DOC_ROOT."/modules/hris/include/vocab.php");
include_once(XOCP_DOC_ROOT."/modules/pms/class/ajax_objective.php");
include_once(XOCP_DOC_ROOT."/modules/pms/pmsjamreport.php");
require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
include_once(XOCP_DOC_ROOT."/modules/pms/class/selectpms.php");

class _pms_JAMReportApproval extends XocpBlock {
   var $catchvar = _PMS_CATCH_VAR;
   var $blockID = _PMS_JAMREPORTAPPROVAL_BLOCK;
   var $width = "100%";
   var $language;
   var $display_title = TRUE;
   var $title = _PMS_JAMREPORTAPPROVAL_BLOCK_TITLE;
   var $display_comment = TRUE;
   var $data;
   
   function __contruct($catch=NULL) { /* fungsi konstruktor wajib punya parameter $catch
   ...
   function requestList() {
      ...
      $sql = "SELECT a.report_approval_st,a.employee_id,b.employee_ext_id,c.person_nm,a.jam_org_ind,a.org_id,a.report_approval2_dttm"
           . " FROM pms_jam a"
           . " LEFT JOIN ".XOCP_PREFIX."employee b USING(employee_id)"
           . " LEFT JOIN ".XOCP_PREFIX."persons c USING(person_id)"
           . " WHERE a.psid = '$psid' AND (a.report_approval1_employee_id = '$self_employee_id' OR a.report_approval2_employee_id = '$self_employee_id')"
           . " AND a.report_approval_st IN ('approval1','approval2','return','final')"
           . " GROUP BY a.employee_id,a.report_approval_st,a.jam_org_ind";
      $result = $db->query($sql);
      $no=0;
      if($db->getRowsNum($result)>0) {
         while(list($report_approval_st,$employee_id,$nip,$employee_nm,$jam_org_ind,$org_id,$report_approval2_dttm)=$db->fetchRow($result)) {
            $no++;
            $jam_org_nm = "";
            $jam_org_class_nm = "";
            if($jam_org_ind==1) {
            ...
            switch($report_approval_st) {
               case "new":
                  $report_approval_st_txt = "Preparation";
                  break;
               case "approval1":
                  $report_approval_st_txt = "Waiting Superior Approval";
                  break;
               case "approval2":
                  $report_approval_st_txt = "Waiting Next Superior Approval";
                  break;
               case "final":
                  $report_approval_st_txt = "Final";
                  break;
               case "return":
                  $report_approval_st_txt = "Returned";
                  break;
               default:
                  $report_approval_st_txt = "Unknown Status";
                  break;
            }
            ...
            $ret .= "<tr>"
                  ...
                  . "<td><a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&goto=y&jam_org_ind=$jam_org_ind&jam_org_id=$org_id&employee_id=$employee_id&u=".uniqid('u')."'>$report_approval_st_txt</a>"
                  . ($report_approval_st=="final"?"&nbsp;".sql2ind($report_approval2_dttm):"")
                  . "</td>"
                  . "</tr>";
         }
      } else {
   ...
   function approval($employee_id,$jam_org_ind,$jam_org_id) {
      ...
      $sql = "SELECT DISTINCT(report_approval_st) FROM pms_jam WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $result = $db->query($sql);
      _debuglog($sql);
      if($db->getRowsNum($result)==1) {
         ...
         if($employee_id!=$self_employee_id) {
            if($jam_status_cd=="new") {
               return $ret. "<br/><br/>JAM report is still prepared.";
            }
         }
      } else {
      ...
      $ret .= _pms_JAMReport::pmsjam($employee_id,$jam_org_ind,$jam_org_id);
      return $ret;
   }
