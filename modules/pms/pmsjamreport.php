...
if ( !defined('PMS_JAMREPORT_DEFINED') ) {
   define('PMS_JAMREPORT_DEFINED', TRUE);
...
class _pms_JAMReport extends XocpBlock {
   var $catchvar = _PMS_CATCH_VAR;
   var $blockID = _PMS_JAMREPORT_BLOCK;
   var $width = "100%";
   var $language;
   var $display_title = TRUE;
   var $title = _PMS_JAMREPORT_BLOCK_TITLE;
   ...
   function pmsjam($employee_id,$jam_org_ind,$jam_org_id) {
      ...
      $sql = "SELECT DISTINCT(report_approval_st) FROM pms_jam WHERE psid = '$psid' AND employee_id = '$employee_id'";
      $result = $db->query($sql);
      ...
      $jam_status_cd = "";
      $report_submit_dttm = "0000-00-00 00:00:00";
      ...
      $report_approval1_employee_idx = $report_approval2_employee_idx = 0;
      
      $sql = "SELECT report_approval1_employee_id,report_approval2_employee_id FROM pms_jam WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $result = $db->query($sql);
      $need_resubmit = 0;
      if($db->getRowsNum($result)>0) {
         while(list($report_approval1_employee_id,$report_approval2_employee_id)=$db->fetchRow($result)) {
            $report_approval1_employee_idx = $report_approval1_employee_id;
            $report_approval2_employee_idx = $report_approval2_employee_id;
            if($report_approval1_employee_id==0) {
               $need_resubmit++;
            }
            if($report_approval2_employee_id==0) {
               $need_resubmit++;
            }
         }
      }
      
      $sql = "SELECT report_approval_st,report_return_note FROM pms_jam WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind' AND org_id = '$jam_org_id'";
      $result = $db->query($sql);
      list($jam_status_cd,$report_return_note)=$db->fetchRow($result);
      if($jam_status_cd=="return") {
         $ret .= "<div style='margin-top:5px;background-color:#ffcccc;-moz-box-shadow:1px 1px 3px #000;border:1px solid #000;-moz-border-radius:5px;padding:5px;color:black;margin-bottom:10px;'><span style='font-weight:bold;'>Returned / Not Approved:</span><div style='white-space:pre-wrap;'>$report_return_note</div></div>";
      }
      ...
      $sql = "SELECT b.pms_objective_no,c.pms_perspective_code,b.pms_org_id,a.pms_objective_id,b.pms_objective_text,a.objective_weight,"
           ...
           . "a.report_approval1_dttm,a.report_approval2_dttm,a.report_approval_st,a.report_submit_dttm,a.final_modified_ind,"
           . "a.reported_final_result,a.kpi_final_result,a.ap_final_result"
           . " FROM pms_jam a"
           . " LEFT JOIN pms_objective b USING(psid,pms_objective_id)"
      ...
      if($db->getRowsNum($result)>0) {
         ...
         while(list($pms_objective_no,$pms_perspective_code,$pms_org_id,$pms_objective_id,$pms_objective_text,$objective_weight,
                    ...
                    $report_approval1_dttm,$report_approval2_dttm,$report_approval_st,$report_submit_dttmx,$final_modified_ind,
                    $reported_final_result,$kpi_final_result,$ap_final_result)=$db->fetchRow($result)) {
            ...
            if($all_jam_status=="new") {
               ...
               if($all_jam_status=="return") {
                  ...
                  if($all_jam_status=="approval1") {
                     if($report_approval_st=="return") $all_jam_status = "return";
                     if($report_approval_st=="new") $all_jam_status = "new";
                  } else {
                     if($all_jam_status=="approval2") {
                        if($report_approval_st=="approval1") $all_jam_status = "approval1";
                        if($report_approval_st=="return") $all_jam_status = "return";
                        if($report_approval_st=="new") $all_jam_status = "new";
                     } else {
                        $all_jam_status = $report_approval_st;
                     }
                  }
               }
            }
            ...
            $jam_status_cd = $report_approval_st;
            $report_submit_dttm = $report_submit_dttmx;
            $first_assessor_approved_dttm = $report_approval1_dttm;
            $next_assessor_approved_dttm = $report_approval2_dttm;
            ...
            list($ach,$kpi_ach)=_pms_calcPeopleAch($employee_id,$pms_objective_id);
            
            
            // _debuglog("$ach / $kpi_ach / $employee_id / $pms_objective_id ::: $pms_objective_no,$pms_perspective_code,$pms_org_id,$pms_objective_id,$pms_objective_text");
            
            if($ach==-999) $ach = 0;
            if($kpi_ach==-999) $kpi_ach = 0;
            
            if($final_modified_ind==0) {
               $ap_final_result = _bctrim(bcdiv(bcmul($ach,$objective_weight),100));
               $kpi_final_result = _bctrim(bcdiv(bcmul($kpi_ach,$objective_weight),100));
               $reported_final_result = $kpi_final_result;
               $sql = "UPDATE pms_jam SET reported_final_result = '$reported_final_result', kpi_final_result = '$kpi_final_result', ap_final_result = '$ap_final_result'"
            ...
            $ttl_kpi_achievement = _bctrim(bcadd($ttl_kpi_achievement,$kpi_final_result));
            $ttl_ap_achievement = _bctrim(bcadd($ttl_ap_achievement,$ap_final_result));
            
            $ret .= "<tr height='75'>";
            $ret .= "<td style='vertical-align:top;border-right:1px solid #bbb;text-align:center;' rowspan='3'>$no</td>";
            $ret .= "<td style='vertical-align:top;border-right:1px solid #bbb;text-align:center;".($top_level_org_id==0?"color:blue;":"")."font-weight:bold;' rowspan='3'>${pms_perspective_code}${pms_objective_no}</td>";
            ...
            $ret .= "<td style='white-space:pre-wrap;vertical-align:top;border-right:1px solid #bbb;'>".(($jam_status_cd=="new"||$jam_status_cd=="return")&&$self_employee_id==$employee_id?"<span >".($target_text0)."</span>":$target_text0)."</td>";
            $ret .= "<td style='white-space:pre-wrap;vertical-align:top;border-right:1px solid #bbb;'>".(($jam_status_cd=="new"||$jam_status_cd=="return")&&$self_employee_id==$employee_id?"<span >".($target_text1)."</span>":$target_text1)."</td>";
            $ret .= "<td style='white-space:pre-wrap;vertical-align:top;border-right:1px solid #bbb;'>".(($jam_status_cd=="new"||$jam_status_cd=="return")&&$self_employee_id==$employee_id?"<span >".($target_text2)."</span>":$target_text2)."</td>";
            $ret .= "<td style='white-space:pre-wrap;vertical-align:top;border-right:1px solid #bbb;'>".(($jam_status_cd=="new"||$jam_status_cd=="return")&&$self_employee_id==$employee_id?"<span >".($target_text3)."</span>":$target_text3)."</td>";
            $ret .= "<td style='white-space:pre-wrap;vertical-align:top;border-right:1px solid #bbb;'>".(($jam_status_cd=="new"||$jam_status_cd=="return")&&$self_employee_id==$employee_id?"<span >".($target_text4)."</span>":$target_text4)."</td>";
            $ret .= "</tr>";
            $ret .= "<tr>";
            ...
            $report_approval2_employee_idx = $next_assessor_employee_id;
            $report_approval1_employee_idx = $assessor_employee_id;
            
            $sql = "UPDATE pms_jam SET "
                 . " report_approval1_employee_id = '$report_approval1_employee_idx',"
                 . " report_approval2_employee_id = '$report_approval2_employee_idx'"
                 . " WHERE psid = '$psid' AND employee_id = '$employee_id' AND jam_org_ind = '$jam_org_ind'";
            $db->query($sql);
            
            if($employee_id==$self_employee_id) {
            
            } else if($assessor_employee_id==$self_employee_id) {
               $final_edit = 1;
            } else if($next_assessor_employee_id==$self_employee_id) {
               $final_edit = 1;
            }
            
            if($jam_status_cd=="final") $final_edit = 0;
            ...
            $norm_result = _bctrim(($kpi_final_result*100)/$objective_weight);
            $bgcolor = "#aaffaa";
            ksort($norm_color);
            foreach($norm_color as $norm_limit=>$color) {
               if($norm_result<$norm_limit) {
                  $bgcolor = $color;
                  break;
               }
            }
            
            $ret .= "<tr style='background-color:${bgcolor};'><td style='border-bottom:1px solid #bbb;text-align:right;font-weight:bold;'>KPI Achievement :</td>"
                  . "<td style='border-bottom:1px solid #bbb;font-weight:bold;border-right:1px solid #bbb;text-align:right;padding:4px;'>"
                  . ($final_edit!=1?"":"<span class='xlnk' onclick='edit_kpi_final_result(\"$pms_objective_id\",this,event);'>")
                  . toMoney($kpi_final_result)
                  . ($final_edit!=1?"":"</span>")
                  . " %</td></tr>";
                  
            $norm_result = _bctrim(($ap_final_result*100)/$objective_weight);
            $bgcolor = "#aaffaa";
            ksort($norm_color);
            foreach($norm_color as $norm_limit=>$color) {
               if($norm_result<$norm_limit) {
            ...
            $ret .= "<tr style='background-color:${bgcolor};'><td style='border-bottom:1px solid #bbb;text-align:right;font-weight:bold;'>Action Plan Achievement :</td>"
                  . "<td style='border-bottom:1px solid #bbb;font-weight:bold;border-right:1px solid #bbb;text-align:right;padding:4px;'>"
                  . ($final_edit!=1?"":"<span class='xlnk' onclick='edit_ap_final_result(\"$pms_objective_id\",this,event);'>")
                  . toMoney($ap_final_result)
                  . ($final_edit!=1?"":"</span>")
                  . " %</td></tr>";
                  
            $norm_result = _bctrim(($reported_final_result*100)/$objective_weight);
            $bgcolor = "#aaffaa";
            ksort($norm_color);
            foreach($norm_color as $norm_limit=>$color) {
               if($norm_result<$norm_limit) {
            ...
            $ret .= "<tr style='background-color:${bgcolor};'><td style='text-align:right;font-weight:bold;'>Final Result :</td><td style='font-weight:bold;border-right:1px solid #bbb;text-align:right;padding:4px;'>"
                  . ($final_edit!=1?"":"<span class='xlnk' onclick='edit_final_result(\"$pms_objective_id\",this,event);'>")
                  . toMoney($reported_final_result)
                  . ($final_edit!=1?"":"</span>")
                  . " %</td></tr>"
                  
                  . "</tbody></table>"
                  . "</td><td colspan='5' style='border-right:1px solid #bbb;padding:0;'>"
                  . "<table style='border-spacing:0px;width:100%;'>"
                  . "<tbody>"
                  . "<tr><td style='border-bottom:1px solid #bbb;padding:4px;'> = ( ".toMoney($kpi_ach)." x ".toMoney($objective_weight)." ) / 100</td></tr>"
                  . "<tr><td style='border-bottom:1px solid #bbb;padding:4px;'> = ( ".toMoney($ach)." x ".toMoney($objective_weight)." ) / 100</td></tr>"
                  . "<tr><td style='border-bottom:0px solid #bbb;padding:4px;'> = ( ".toMoney($kpi_ach)." x ".toMoney($objective_weight)." ) / 100</td></tr>"
                  . "</tbody></table>"
                  . "</td></tr>";
      ...
      $ret .= "<td style='border-top:1px solid #bbb;background-color:#eee;color:black;border-right:1px solid #bbb;text-align:right;font-weight:bold;'>&nbsp;</td>";
      ...
      $ret .= "<div style='margin-top:20px;'>"
            . "<table align='center' style='background-color:${ttl_bgcolor};font-size:1.5em;width:300px;-moz-border-radius:5px;border:2px solid #888;'>"
            . "<tbody><tr><td style='padding:10px;'>"
            . "Total Achivement : "
            . "</td><td id='ttl_achievement'>".toMoney($ttl_achievement)." %</td></tr>"
            . "</tbody></table></div>";
      ...
      $form .= "<div style='text-align:right;padding:10px;margin-top:20px;'>"
             ...
             . (!($all_jam_status==""||$all_jam_status=="new"||$all_jam_status=="return")?"Submited at:<br/>".sql2ind($report_submit_dttm,"date"):"")
             ...
             . ($all_jam_status=="final"?"Approved at:<br/>".sql2ind($first_assessor_approved_dttm,"date"):"")
             . "</td>"
             
             . ($doubleapproval==1?""
             . "<td style='text-align:center;border-bottom:2px solid #777;border-right:2px solid #777;padding:10px;'>"
             . ($all_jam_status=="new"||$all_jam_status=="return"?"-":"")
             . ($all_jam_status=="approval2"&&$self_employee_id==$report_approval2_employee_idx?"<input type='button' value='Approve' onclick='confirm_approval2(this,event);'/>&nbsp;<input type='button' value='Not Approve' onclick='next_assessor_return_JAM(\"$employee_id\");'/>":"")
             . ($all_jam_status=="approval2"&&$self_employee_id!=$report_approval2_employee_idx?"Waiting for approval $self_employee_i":"")
             . ($all_jam_status=="approval1"&&$self_employee_id!=$report_approval2_employee_idx?"-":"")
             . ($all_jam_status=="approval1"&&$self_employee_id==$report_approval2_employee_idx?"-":"")
             . ($all_jam_status=="final"?"Approved at:<br/>".sql2ind($next_assessor_approved_dttm,"date"):"")
             . ($all_jam_status=="final"&&$_SESSION["allow_undo_jam_report"]==1?"<div style='margin-top:5px;'><input type='button' value='Undo' onclick='undo_next_approval(this,event);'/></div>":"")
             . "</td>"
             . "":"")
      ...
      function do_approval2(employee_id) {
         ...
         orgjx_app_approval2JAMReport('$employee_id','$jam_org_ind','$jam_org_id',function(_data) {
      ...
      function confirm_next_assessor_return_JAM(employee_id) {
         var report_return_note = urlencode($('report_return_note').value);
         orgjx_app_nextAssessorReturnJAMReport('$employee_id','$jam_org_ind','$jam_org_id',report_return_note,function(_data) {
      ...
      function next_assessor_return_JAM(employee_id) {
         ...
         var xreport_return_note = '';
         if($('xreport_return_note')) {
            xreport_return_note = $('xreport_return_note').innerHTML;
         }
         ...
         $('innernextassessorreturnedit').innerHTML = '<div style=\"height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;\">Not Approve JAM Confirmation</div>'
                                   ...
                                   + '<textarea id=\"report_return_note\" style=\"width:400px;height:100px;\">'+xreport_return_note+'</textarea>'
                                   + '<br/><br/>Are you sure?'
                                   + '<br/><br/>'
                                   ...
                                   + '</div>'
                                   + '</div>';
         setTimeout('$(\"report_return_note\").focus();',200);
      }
      ...
      function do_undo_next_approval(employee_id) {
         $('confirmbtn').innerHTML = '';
         $('confirmbtn').appendChild(progress_span(' ... approve JAM'));
         orgjx_app_undoNextApprovalJAMReport('$employee_id','$jam_org_ind','$jam_org_id',function(_data) {
            undonextapprovalbox.fade();
            location.href = '".XOCP_SERVER_SUBDIR."/index.php?goto=y&jam_org_ind=${jam_org_ind}&jam_org_id=${jam_org_id}&employee_id=${employee_id}&r='+uniqid('r');
         });
      }
      ...
      var undonextapproval = null;
      var undonextapprovalbox = null;
      function undo_next_approval(d,e) {
         undonextapproval = _dce('div');
         undonextapproval.setAttribute('id','undonextapproval');
         undonextapproval = document.body.appendChild(undonextapproval);
         undonextapproval.sub = undonextapproval.appendChild(_dce('div'));
         undonextapproval.sub.setAttribute('id','innerundonextapproval');
         undonextapprovalbox = new GlassBox();
         $('innerundonextapproval').innerHTML = '<div style=\"height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;\">Undo JAM Report Confirmation</div>'
                                           + '<div id=\"confirmmsg\" style=\"padding:20px;text-align:center;\">Are you going to undo this JAM approval?</div>'
                                           + '<div id=\"confirmbtn\" style=\"background-color:#eee;padding:10px;text-align:center;\">'
                                             + '<input type=\"button\" value=\"Yes (Undo)\" onclick=\"do_undo_next_approval();\"/>&nbsp;&nbsp;'
                                             + '<input type=\"button\" value=\"No (Cancel)\" onclick=\"undonextapprovalbox.fade();\"/>'
                                           + '</div>';
         
         
         undonextapprovalbox = new GlassBox();
         undonextapprovalbox.init('undonextapproval','500px','165px','hidden','default',false,false);
         undonextapprovalbox.lbo(false,0.3);
         undonextapprovalbox.appear();
      }
      ...
      function do_approval1(employee_id) {
         $('confirmbtn').innerHTML = '';
         $('confirmbtn').appendChild(progress_span(' ... approve JAM'));
         orgjx_app_approval1JAMReport('$employee_id','$jam_org_ind','$jam_org_id',function(_data) {
      ...
      function confirm_first_assessor_return_JAM(employee_id) {
         var report_return_note = urlencode($('report_return_note').value);
         orgjx_app_firstAssessorReturnJAMReport('$employee_id','$jam_org_ind','$jam_org_id',report_return_note,function(_data) {
            var data = recjsarray(_data);
            location.href = '".XOCP_SERVER_SUBDIR."/index.php?goto=y&jam_org_ind=${jam_org_ind}&jam_org_id=${jam_org_id}&employee_id=${employee_id}&r='+uniqid('r');
         });
      }
      ...
      function do_submit(employee_id) {
         ...
         orgjx_app_submitJAMReport('$employee_id',function(_data) {
      ...
      function confirm_submit(d,e) {
         ...
         $('innerconfirmsubmit').innerHTML = '<div style=\"height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;\">Submit JAM Report Confirmation</div>'
                                           + '<div id=\"confirmmsg\" style=\"padding:20px;text-align:center;\">Are you going to submit this JAM Report?</div>'
                                           + '<div id=\"confirmbtn\" style=\"background-color:#eee;padding:10px;text-align:center;\">'
      ...
      ////////////////////////////
      var dveditfinalresult = null;
      function save_final_result(pms_objective_id,no) {
         var val = trim($('inp_final_result').value);
         if(dveditfinalresult) {
            dveditfinalresult.d.innerHTML = val;
         }
         ajax_feedback = _caf;
         orgjx_app_saveJAMFinalResult(val,pms_objective_id,'$employee_id',function(_data) {
            var data = recjsarray(_data);
            $('ttl_achievement').innerHTML = data[1];
            $('ttl_achievement').parentNode.parentNode.parentNode.style.backgroundColor = data[3];
            if(dveditfinalresult) {
               dveditfinalresult.d.innerHTML = data[0];
               dveditfinalresult.d.parentNode.parentNode.style.backgroundColor = data[2];
            }
            close_final_result();
         });
      }
      
      function reset_final_result(pms_objective_id,no) {
         ajax_feedback = _caf;
         orgjx_app_resetJAMFinalResult(pms_objective_id,'$employee_id',function(_data) {
            $('inp_final_result').value = _data;
            if(dveditfinalresult) {
               dveditfinalresult.d.innerHTML = _data;
            }
         });
      }
      
      function btn_reset_final_result() {
         reset_final_result(dveditfinalresult.pms_objective_id,0);
      }
      
      function btn_save_final_result() {
         save_final_result(dveditfinalresult.pms_objective_id,0);
      }
      
      function kp_final_result(d,e) {
         var k = getkeyc(e);
         if(d.chgt) {
            d.chgt.reset();
            d.chgt = null;
         ...
         if(k==13) {
            dveditfinalresult.d.innerHTML = val;
            save_final_result(dveditfinalresult.pms_objective_id,dveditfinalresult.no);
         } else if (k==27) {
            _destroy(dveditfinalresult);
            dveditfinalresult.d = null;
            dveditfinalresult = null;
         }
      }
      
      function close_final_result() {
         document.body.onclick = null;
         _destroy(dveditfinalresult);
         dveditfinalresult.d = null;
         dveditfinalresult = null;
      ...
      /// var dveditfinalresult = null;
      function edit_final_result(pms_objective_id,d,e) {
         document.body.onclick = null;
         _destroy(dveditfinalresult);
         if(dveditfinalresult&&d==dveditfinalresult.d) {
            dveditfinalresult.d = null;
            dveditfinalresult = null;
            return;
         }
         ...
         d.dv.innerHTML = '<div style=\"text-align:left;padding:2px;\">Final Result :<br/>'
                        + '<input type=\"text\" onkeyup=\"kp_final_result(this,event);\" id=\"inp_final_result\" style=\"text-align:center;\" value=\"'+text+'\">'
                        + '<div style=\"margin-top:5px;text-align:right;\">'
                        + '<input class=\"sbtn\" type=\"button\" value=\"Save\" onclick=\"btn_save_final_result();\"/>'
                        + '&nbsp;<input class=\"sbtn\" type=\"button\" value=\"Reset\" onclick=\"btn_reset_final_result();\"/>'
                        + '&nbsp;<input class=\"sbtn\" type=\"button\" value=\"Close\" onclick=\"close_final_result();\"/>'
                        + '</div>'
                        + '</div>';
         d.dv = d.parentNode.appendChild(d.dv);
         d.dv.style.top = parseInt(oY(d.parentNode)+d.parentNode.offsetHeight+7)+'px';
         var x = oX(d);
         if(x>650) {
            d.dv.style.left = parseInt(oX(d.parentNode)-(d.dv.offsetWidth)+(d.parentNode.offsetWidth))+'px';
         } else {
         ...
         setTimeout('_dsa($(\"inp_final_result\"))',10);
         dveditfinalresult = d.dv;
         dveditfinalresult.d = d;
         dveditfinalresult.pms_objective_id = pms_objective_id;
         //setTimeout('document.body.onclick = function() { document.body.onclick = null; _destroy(dveditfinalresult); };',100);
      }
      
      ////////////////////////////
      
      ////////////////////////////
      function save_kpi_final_result(pms_objective_id,no) {
         var val = trim($('inp_kpi_final_result').value);
         if(dveditfinalresult) {
            dveditfinalresult.d.innerHTML = val;
         }
         ajax_feedback = _caf;
         orgjx_app_saveJAMKPIFinalResult(val,pms_objective_id,'$employee_id',function(_data) {
            var data = recjsarray(_data);
            if(dveditfinalresult) {
               dveditfinalresult.d.innerHTML = data[0];
               dveditfinalresult.d.parentNode.parentNode.style.backgroundColor = data[1];
            }
            close_kpi_final_result();
         });
      }
      
      function reset_kpi_final_result(pms_objective_id,no) {
         ajax_feedback = _caf;
         orgjx_app_resetJAMKPIFinalResult(pms_objective_id,'$employee_id',function(_data) {
            $('inp_kpi_final_result').value = _data;
         });
      }
      
      function btn_reset_kpi_final_result() {
         reset_kpi_final_result(dveditfinalresult.pms_objective_id,0);
      }
      
      function btn_save_kpi_final_result() {
         save_kpi_final_result(dveditfinalresult.pms_objective_id,0);
      }
      
      function kp_kpi_final_result(d,e) {
         var k = getkeyc(e);
         if(d.chgt) {
            d.chgt.reset();
            d.chgt = null;
         ...
         if(k==13) {
            dveditfinalresult.d.innerHTML = val;
            save_kpi_final_result(dveditfinalresult.pms_objective_id,dveditfinalresult.no);
         } else if (k==27) {
            _destroy(dveditfinalresult);
            dveditfinalresult.d = null;
            dveditfinalresult = null;
      ...
      function close_kpi_final_result() {
         document.body.onclick = null;
         _destroy(dveditfinalresult);
         dveditfinalresult.d = null;
         dveditfinalresult = null;
      ...
      /// var dveditfinalresult = null;
      function edit_kpi_final_result(pms_objective_id,d,e) {
         document.body.onclick = null;
         _destroy(dveditfinalresult);
         if(dveditfinalresult&&d==dveditfinalresult.d) {
            dveditfinalresult.d = null;
         ...
         d.dv.innerHTML = '<div style=\"text-align:left;padding:2px;\">KPI Final Result :<br/>'
                        + '<input type=\"text\" onkeyup=\"kp_kpi_final_result(this,event);\" id=\"inp_kpi_final_result\" style=\"text-align:center;\" value=\"'+text+'\">'
                        + '<div style=\"margin-top:5px;text-align:right;\">'
                        + '<input class=\"sbtn\" type=\"button\" value=\"Save\" onclick=\"btn_save_kpi_final_result();\"/>'
                        + '&nbsp;<input class=\"sbtn\" type=\"button\" value=\"Reset\" onclick=\"btn_reset_kpi_final_result();\"/>'
                        + '&nbsp;<input class=\"sbtn\" type=\"button\" value=\"Close\" onclick=\"close_kpi_final_result();\"/>'
                        + '</div>'
                        + '</div>';
         d.dv = d.parentNode.appendChild(d.dv);
         d.dv.style.top = parseInt(oY(d.parentNode)+d.parentNode.offsetHeight+7)+'px';
         ...
         setTimeout('_dsa($(\"inp_kpi_final_result\"))',10);
         dveditfinalresult = d.dv;
         dveditfinalresult.d = d;
         dveditfinalresult.pms_objective_id = pms_objective_id;
         //setTimeout('document.body.onclick = function() { document.body.onclick = null; _destroy(dveditfinalresult); };',100);
      ...
      ////////////////////////////
      function save_ap_final_result(pms_objective_id,no) {
         var val = trim($('inp_ap_final_result').value);
         if(dveditfinalresult) {
            dveditfinalresult.d.innerHTML = val;
         }
         ajax_feedback = _caf;
         orgjx_app_saveJAMAPFinalResult(val,pms_objective_id,'$employee_id',function(_data) {
            var data = recjsarray(_data);
            if(dveditfinalresult) {
               dveditfinalresult.d.innerHTML = data[0];
               dveditfinalresult.d.parentNode.parentNode.style.backgroundColor = data[1];
            }
            close_ap_final_result();
         });
      }
      
      function reset_ap_final_result(pms_objective_id,no) {
         ajax_feedback = _caf;
         orgjx_app_resetJAMAPFinalResult(pms_objective_id,'$employee_id',function(_data) {
            $('inp_ap_final_result').value = _data;
            if(dveditfinalresult) {
               dveditfinalresult.d.innerHTML = _data;
            }
         });
      ...
      function btn_reset_ap_final_result() {
         reset_ap_final_result(dveditfinalresult.pms_objective_id,0);
      }
      
      function btn_save_ap_final_result() {
         save_ap_final_result(dveditfinalresult.pms_objective_id,0);
      }
      
      function kp_ap_final_result(d,e) {
         var k = getkeyc(e);
         if(d.chgt) {
            d.chgt.reset();
            d.chgt = null;
         ...
         if(k==13) {
            dveditfinalresult.d.innerHTML = val;
            save_ap_final_result(dveditfinalresult.pms_objective_id,dveditfinalresult.no);
         } else if (k==27) {
            _destroy(dveditfinalresult);
            dveditfinalresult.d = null;
            dveditfinalresult = null;
      ...
      function close_ap_final_result() {
         document.body.onclick = null;
         _destroy(dveditfinalresult);
         dveditfinalresult.d = null;
         dveditfinalresult = null;
      ...
      var dveditfinalresult = null;
      function edit_ap_final_result(pms_objective_id,d,e) {
         document.body.onclick = null;
         _destroy(dveditfinalresult);
         if(dveditfinalresult&&d==dveditfinalresult.d) {
            dveditfinalresult.d = null;
         ...
         d.dv.innerHTML = '<div style=\"text-align:left;padding:2px;\">Action Plan Final Result :<br/>'
                        + '<input type=\"text\" onkeyup=\"kp_ap_final_result(this,event);\" id=\"inp_ap_final_result\" style=\"text-align:center;\" value=\"'+text+'\">'
                        + '<div style=\"margin-top:5px;text-align:right;\">'
                        + '<input class=\"sbtn\" type=\"button\" value=\"Save\" onclick=\"btn_save_ap_final_result();\"/>'
                        + '&nbsp;<input class=\"sbtn\" type=\"button\" value=\"Reset\" onclick=\"btn_reset_ap_final_result();\"/>'
                        + '&nbsp;<input class=\"sbtn\" type=\"button\" value=\"Close\" onclick=\"close_ap_final_result();\"/>'
                        + '</div>'
                        + '</div>';
         d.dv = d.parentNode.appendChild(d.dv);
         d.dv.style.top = parseInt(oY(d.parentNode)+d.parentNode.offsetHeight+7)+'px';
         ...
         setTimeout('_dsa($(\"inp_ap_final_result\"))',10);
         dveditfinalresult = d.dv;
         dveditfinalresult.d = d;
         dveditfinalresult.pms_objective_id = pms_objective_id;
         //setTimeout('document.body.onclick = function() { document.body.onclick = null; _destroy(dveditfinalresult); };',100);
