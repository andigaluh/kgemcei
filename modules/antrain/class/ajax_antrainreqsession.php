...
class _antrain_class_ANTRAINReqsessionAjax extends AjaxListener {
   
   function __construct($act_name) {
      $this->_act_name = $act_name;
      $this->_include_file = XOCP_DOC_ROOT."/modules/antrain/class/ajax_antrainreqsession.php";
      $this->init();
      parent::init();
   }
   ...
   function app_newSession($args) {
     ...
     $result = $db->query($sql);
	  ...
	  $sql = "INSERT INTO antrain_sessionreq (psid,year,budget,remark,id_created,date_created,org_id)"
...
/* 	$sql = "SELECT year,budget,remark,date_created"
  ...
  $sqlselected = "SELECT  org_id,org_id_sec FROM antrain_sessionreq WHERE psid = $psid";
