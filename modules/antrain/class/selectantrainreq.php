...
class _antrain_class_SelectSession extends XocpAttachable {
   ...
   $resultjob = $db->query($sqljob);
    list($job_id,$org_id)=$db->fetchRow($resultjob);
      ...
      if ($job_id == 146  || $job_id == 999 || $job_id == 130 || $job_id == 68 || $job_id == 147  || $job_id == 101 || $job_id == 90 || $job_id == 6 || $job_id == 0)
      {
         $divsecid = '';
        $divsecidx = '';
      }   
      ...
      $sqlyear = "SELECT id FROM hris_global_sess WHERE status_cd = 'normal'";
     ...
     $sql = "SELECT p.is_proposed, p.date_proposed,  pk.org_nm, pk.org_class_id FROM antrain_sessionreq p LEFT JOIN antrain_budget pb ON pb.id = p.id_hris_budget LEFT JOIN hris_orgs pk ON pb.org_id = pk.org_id WHERE psid = ('$psid')" ;
