<?php
//--------------------------------------------------------------------//
// Filename : modules/hris/class/ajax_ss_cancelalter.php                   //
...
if ( !defined('HRIS_SSCANCELALTERAJAX_DEFINED') ) {
   define('HRIS_SSCANCELALTERAJAX_DEFINED', TRUE);
...
class _antrain_class_SsCancelalterAjax extends AjaxListener {
   ...
   function _antrain_class_SsCancelalterAjax($act_name) {
      ...
      $this->_include_file = XOCP_DOC_ROOT."/modules/antrain/class/ajax_ss_cancelalter.php";
      $this->init();
      parent::init();
   }
   ...
   function init() {
      $this->registerAction($this->_act_name,"app_Delete","app_editSession","app_saveSession",
                                             "app_setANTRAINSession","app_newSession" ,"app_propose","app_proposediv","app_approve" ,"app_returnSession", "app_saveReturn" ,"app_returnSessionDivmjr", "app_saveReturnDivmjr" ,"app_returnSessionDir", "app_saveReturnDir" , "app_returnSessionApproval", "app_saveReturnApproval", "app_reminder", "app_send_reminder","app_reminder_omsm","app_send_reminder_omsm");
   }
...
/* 	$sql = "SELECT year,budget,remark,date_created"
   ...
   function app_send_reminder() {
	...
	//cc mail list
	$sqlcc = "SELECT email FROM antrain_ccmail_return";
...
//		return array($psid,$id_propose,$is_propose,$date_propose,$proposed_by);
   ...
   function app_proposediv($args) {
	...
	include_once('Mail.php');
		...
		//$sql = "SELECT id_proposed, is_proposed, date_proposed FROM antrain_sessionss WHERE psid= '$psid' ORDER BY year";
		//$db->query($sql);	
...
//$mail_object->send($recipient, $headers, $body);
        ...
        $sql = "UPDATE antrain_sessionss SET id_proposeddiv = '$id_proposeddiv', is_proposeddiv = $is_proposeddiv, date_proposeddiv = '$date_proposeddiv' WHERE psid = '$psid'";
        $db->query($sql);
...
//		return array($psid,$id_propose,$is_propose,$date_propose,$proposed_by);
	...
	$subject = 	"Reminder $namato";
         ...
         $headers["To"]      = $emailto;
		 $headers["Cc"] =  $emailcc;  
         $headers["Subject"] = "[HRIS TRAINING APPROVED] ". $subject;
         $headers["MIME-Version"] = "1.0";
         $headers["Content-type"] = "text/html; charset=iso-8859-1";
         $body = 	"<!DOCTYPE html PUBLIC '-//W3C//DTD XHTML 1.0 Transitional//EN' 'http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd'><html xmlns='http://www.w3.org/1999/xhtml'>
									<head>
...
/* 	$sql = "SELECT year,budget,remark,date_created"
	...
	$psid = $args[0];
	 ...
	 $sqlto = "SELECT p.user_id,ps.email,ps.person_nm,pk.date_proposed FROM hris_users p LEFT JOIN hris_persons ps ON p.person_id = ps.person_id LEFT JOIN antrain_sessionss pk ON p.user_id = pk.id_created WHERE pk.psid = $psid";  
     $resultto = $db->query($sqlto);
...
//	 mail("prince.fachmy@gmail.com","subject","pesan");
	...
	$psid = $args[0];
	 ...
	 $sqlto = "SELECT p.user_id,ps.email,ps.person_nm,pk.date_proposed FROM hris_users p LEFT JOIN hris_persons ps ON p.person_id = ps.person_id LEFT JOIN antrain_sessionss pk ON p.user_id = pk.id_proposed WHERE pk.psid = $psid";  
     $resultto = $db->query($sqlto);
     list($user_id_proposed,$emailto,$namato,$date_proposed)=$db->fetchRow($resultto);
...
//	 mail("prince.fachmy@gmail.com","subject","pesan");
	...
	$psid = $args[0];
	 ...
	 $sqlto = "SELECT p.user_id,ps.email,ps.person_nm,pk.date_proposed FROM hris_users p LEFT JOIN hris_persons ps ON p.person_id = ps.person_id LEFT JOIN antrain_sessionss pk ON p.user_id = pk.id_proposeddiv WHERE pk.psid = $psid";  
     $resultto = $db->query($sqlto);
     list($user_id_proposed,$emailto,$namato,$date_proposed)=$db->fetchRow($resultto);
