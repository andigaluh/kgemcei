<?php
//--------------------------------------------------------------------//
// Filename : modules/antrain/antraincancelalterview.php                         //
...
include_once(XOCP_DOC_ROOT."/modules/antrain/class/selectantraincancelalterview.php");

class _antrain_Cancelalterview extends XocpBlock {
   ...
   function antraincancelalterview() {
	...
	if( $get_userid == 1043 || $get_userid == 1)
 	{
	...
	if( $get_userid == 1040 || $get_userid == 1 )
 	{
	...
	if( $get_userid == 1312 || $get_userid == 1 )
 	{
	...
	$tooltip = "<div id='all_ap_tooltip' style='display:none;'>";
     ...
     /*  $ret .= $return_notes;
      ...
      $ret .= "<div style='padding:5px;font-size:20px;color: #666666; text-align:center;'>TRAINING  CANCELLATION/ALTERATION</div>"
			 . "<div style='padding:5px;font-size:15px;color: #666666; text-align:center;'>(General Subject /Year $year ".getuserid()."  )</div>"
		  ...
		  else
		  {
			...
			//edit by andi
			  $ret .= "<tr><td id='tdclass_${id}' style='margin: 0px; padding:0px;' colspan=10 >"
				  ...
				  . "<td><span id='sp_${id}'>".$stat_can."</span></td>"
                  . "</tr></tbody></table>"
                  . "</td></tr>";
 ...
 var wdv = null;
   ...
   function main() {
      ...
      switch ($this->catch) {
         case $this->blockID:
            $ret = $this->antraincancelalterview($self_employee_id);
            break;
         default:
            $ret = $this->antraincancelalterview($self_employee_id);
            break;
