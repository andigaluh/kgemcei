<?php
//--------------------------------------------------------------------//
// Filename : modules/pms/antrainplan.php                         //
...
class _antrain_Plan extends XocpBlock {
   ...
   function antrainplan($employee_id,$report_mode=FALSE,$force_month_id=0) {
      ...
      $sql = "SELECT p.session_periode, pk.org_nm FROM antrain_session p"
			  . " LEFT JOIN hris_orgs pk ON p.org_id = pk.org_id"
			  . " WHERE psid = ($psid) ORDER BY session_periode DESC " ;
	  
	  /* "SELECT session_periode,session_nm FROM antrain_session WHERE psid  = '$psid'"; */
      $result = $db->query($sql);
      list($session_periode,$session_nm)=$db->fetchRow($result);
      ...
      $ret .= "<div style='padding:5px;font-size:20px;color: #666666; text-align:center;'>ANNUAL TRAINING / SEMINAR PLAN</div>"
			 . "<div style='padding:5px;font-size:15px;color: #666666; text-align:center;'>(General Subject /Year $session_periode )</div>"
			 . "<div style='padding:2px;font-size:12px;color: #666666; text-align:left;'>Division/ Section:  $session_nm </div>"

			  . "<colgroup><col width='3%'/><col width='14%'/><col width='18%'/><col width='8%'/><col width='12%'/><col width='12%'/><col width='12%'/><col width='7%'/><col width='7%'/><col width='10%'/></colgroup>"
			...
			. "<tr><td style='border-right: 1px solid #CCC;  text-align:center;' rowspan='2'>Name</td><td style='border-right: 1px solid #CCC; text-align:center;' rowspan='2'>Position</td><td style='border-right: 1px solid #CCC;  text-align:center;' rowspan='2'>Subject</td><td style='border-right: 1px solid #CCC;  text-align:center;' rowspan='2'>Objective</td><td style='border-right: 1px solid #CCC; text-align:center;' rowspan='2'>Schedule</td><td style='border-right: 1px solid #CCC; text-align:center;' rowspan='2'>Estimation Cost</td><td style='border-right: 1px solid #CCC; text-align:center;' colspan='2'>Instructor*</td></tr>"
      ...
      if($db->getRowsNum($result)>0) {
         ...
         while(list($pms_objective_no,$pms_perspective_code,$pms_org_id,$pms_objective_id,$pms_objective_text,$objective_weight,
            ...
            $ret .= "<td style='border-right:0px solid #bbb;' ".($report_mode==TRUE?"colspan='7'":"colspan='15'")."><span style='color:black;font-weight:bold;'>$pms_objective_text</span>${apdiv}</td>";
            $ret .= "</tr>";
            ...
            $ret .= "<td style='padding:0px;border-right:0px solid #bbb;vertical-align:top;overflow:hidden;' ".($report_mode==TRUE?"":"rowspan='2'")." id='tdtg_${pms_objective_id}'>".$apdata[2]."</td>";
            $ret .= "</tr>";
      ...
      if($report_mode==TRUE) {
         ...
         $ret .= "<td style='border-top:1px solid #bbb;background-color:#eee;color:black;border-right:0px solid #bbb;text-align:center;font-weight:bold;'>$zone_ttl_txt</td>";
         $ret .= "</tr>";
      }
   ...
   function main() {
      $pmsselobj = new _antrain_class_SelectSession();
