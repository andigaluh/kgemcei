...
class _hris_IDPReviewTool extends _hris_AssessmentResult {
                  ...
                  if($db->getRowsNum($res2)>0) {
                     while(list($employee_id,$nip,$employee_nm,$gradeval,$dob,$pob,$gender,$addr,$cellphone,$phone,$marital,
                        ...
                        $sql = "SELECT request_id,status_cd,request_t FROM ".XOCP_PREFIX."idp_request"
                             ...
                             . " AND status_cd NOT IN('nullified') ORDER BY request_id DESC";
                        $rreq = $db->query($sql);
                        $req_count = 0;
                        $req_status = "";
                        ...
                        if($db->getRowsNum($rreq)>0) {
                           while(list($request_id,$status_cd,$request_t)=$db->fetchRow($rreq)) {
                               ...
                               /*$sql = "SELECT request_id,actionplan_id,method_subject FROM ".XOCP_PREFIX."idp_request_actionplan"
                                     . " WHERE request_id = '$request_id'"
                                     . " ORDER BY actionplan_id DESC LIMIT 0,1";
                                    $rreq = $db->query($sql);
                                    if($db->getRowsNum($rreq)>0) {
                                      list($request_id,$actionplan_id,$method_subject)=$db->fetchRow($rreq);
                                    }else{
                                      $request_id = "-";
                                      $actionplan_id = "-";
                                      $method_subject = "NULL";
                              ...
                              switch($status_cd) {
                                 case "start":
                                    $nomer = $nomer + 1;
                                    $method_subject = $this->get_actionplan($request_id);
                                    $req_status .= "  <a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&req=y&e=${employee_id}&j=${job_id}&r=${request_id}' > New Request</a><span style='margin-left:35px'>[$nomer]</span><br/>";
                                    break;
                                 case "employee":
                                    $method_subject = $this->get_actionplan($request_id);
                                    $req_status .= "  <a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&req=y&e=${employee_id}&j=${job_id}&r=${request_id}'> Employee Request </a><br/>";
                                    break;
                                 case "approval1":
                                    $method_subject = $this->get_actionplan($request_id);
                                    $req_status .= "  <a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&req=y&e=${employee_id}&j=${job_id}&r=${request_id}'> Need Approval </a><br/>";
                                    break;
                                 case "approval2":
                                    $method_subject = $this->get_actionplan($request_id);
                                    $req_status .= "  <a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&req=y&e=${employee_id}&j=${job_id}&r=${request_id}'> Next Superior Approval </a><br/>";
                                    break;
                                 case "approval3":
                                    $method_subject = $this->get_actionplan($request_id);
                                    $req_status .= "  <a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&req=y&e=${employee_id}&j=${job_id}&r=${request_id}'> HR Approval </a><br/>";
                                    break;
                                 case "implementation":
                                    $method_subject = $this->get_actionplan($request_id);
                                    $req_status .= "  <a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&req=y&e=${employee_id}&j=${job_id}&r=${request_id}'> Implementation </a><br/>";
                                    break;
                                 case "completed":
                                    $method_subject = $this->get_actionplan($request_id);
                                    $req_status .= "  <p style='margin:10px 0px;'><a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&req=y&e=${employee_id}&j=${job_id}&r=${request_id}'> Completed </a><br/>";
                                    break;
                                 default:
                                    break;
                        ...
                        $employee_list .= "<tr id='tremp_${employee_id}_${job_id}'>"
                                          ...
                                          . "<td style='text-align:left; '>"
                                             ...
                                             //.($req_count>0?$req_status:"Click to start")."&nbsp;"
                                          . "</td>"
                                          . "</tr>";
                        $no++;
                     }
   ...
   function get_actionplan($request_id){
      $db=&Database::getInstance();
      //SELECT a.request_id,a.actionplan_id,a.method_subject,b.method_nm FROM hris_idp_request_actionplan a 
      //LEFT JOIN hris_idp_development_method b ON a.method_id = b.method_id 
      //WHERE a.request_id = '131' ORDER BY a.actionplan_id DESC LIMIT 0,1;
      $sql = "SELECT a.request_id,a.actionplan_id,a.method_subject,b.method_nm FROM ".XOCP_PREFIX."idp_request_actionplan a"
       . " LEFT JOIN ".XOCP_PREFIX."idp_development_method b ON a.method_id = b.method_id"
       . " WHERE a.request_id = '$request_id'"
       . " ORDER BY a.actionplan_id DESC LIMIT 0,1";
      $rreq = $db->query($sql);
      if($db->getRowsNum($rreq)>0) {
        list($request_id,$actionplan_id,$method_subject,$method_nm)=$db->fetchRow($rreq);
        $method_nm = ($request_id == '' ? '' : '['.$request_id.']');
        return $method_nm;
      }else{
        return '';
        /*$request_id = "-";
      ...
      switch ($this->catch) {
         case $this->blockID:
            if(isset($_GET["smethod"])&&$_GET["smethod"]==1&&isset($_GET["t"])&&$_GET["t"]!="") {
               ...
               $ret = _idp_view_request_byhrmonitoring($employee_id,$job_id,TRUE,FALSE,$request_id);
