...
class _fpdf_AssessmentResult extends FPDF {
   ...
   function competency_profile() {
      ...
      $sql = "SELECT b.compgroup_id,a.competency_id,a.rcl,a.itj,b.competency_nm,c.compgroup_nm,b.competency_class,d.ccl,(b.competency_class+0) as urcl,"
           ...
           . " LEFT JOIN ".XOCP_PREFIX."employee_competency_final d ON d.asid = '$asid' AND d.employee_id = '$employee_id' AND d.competency_id = b.competency_id"
      ...
      if($db->getRowsNum($result)>0) {
         ...
         while(list($compgroup_id,$competency_id,$rcl,$itj,$competency_nm,$compgroup_nm,$cc,$cclxxx,$urcl,$asr_nm,$desc_en,$desc_id)=$db->fetchRow($result)) {
            ...
            list($arrccl,$arrasr,$calc_ccl,$org_calc_ccl,$arravg,$arrcclxxx,$calc_cclxxx,$arravgxxx) = _get_arrccl($asid,$employee_id,$competency_id,$job_id);
            
            if($final_modified==1) {
               $calc_cclxxx = $final_cclxxx;
            }
            ...
            $this->Cell($col_val,$ch,"$rcl","TLR",0,"C");
            $this->Cell($col_sep,$ch,"",0,0);
            $this->Cell($col_val,$ch,(count($arrcclxxx)>0?number_format("$calc_cclxxx",2,".",""):"-"),"TLR",0,"C");
            $this->Cell($col_sep,$ch,"",0,0);
            $this->Cell($col_val,$ch,(TRUE>0?($rcl*$itj):"-")   ,"TLR",0,"C");
            $this->Cell($col_val,$ch,(count($arrcclxxx)>0?number_format(($calc_cclxxx*$itj),2,".",""):"-"),"TLR",0,"C");
            $this->Cell($col_val,$ch,(count($arrcclxxx)>0?number_format($gapx,2,".",""):"-"),"TLR",0,"C");
