...
";
   ...
   function _idp_view_request($employee_id,$job_id,$back_button=TRUE,$new_button=TRUE) {
      ...
      if($db->getRowsNum($result)>0) {
         while(list($request_id,$request_t,$timeframe_start,$timeframe_stop,$status_cd)=$db->fetchRow($result)) {
            ...
            $idplist .= "<div id='xidp_${request_id}' style='padding:5px;border-bottom:1px solid #bbb;'>$request</div>";
         }
         if($cnt>0) {
      ...
      $ret = "<div style='max-width:980px;'><div>"
           ...
           . "<table style='width:100%;' class='xxlist'><thead><tr>"
               . "<td style='text-align:center;background-color:#eee;font-weight:bold;border-bottom:0px;'>List of IDP <input type='button' value='New IDP' style='float:right;' onclick='new_idp();'/></td>"
           . "</tr></thead><tbody>"
      ...
      function delete_competency(request_id,competency_id,competency_abbr,competency_nm) {
         ...
         compdeledit.setAttribute('id','compdeledit');
      ...
      */
      ...
      var showbudget = null;
      var showbudgetbox = null;
      function show_budget(person_id,org_id,year_now,d,e) {
         showbudget = _dce('div');
         showbudget.setAttribute('id','showbudget');
         showbudget = document.body.appendChild(showbudget);
         showbudget.sub = showbudget.appendChild(_dce('div'));
         showbudget.sub.setAttribute('id','innershowbudget');
         showbudgetbox = new GlassBox();
         showbudgetbox.init('showbudget','700px','270px','hidden','default',false,false);
         showbudgetbox.lbo(false,0.3);
         showbudgetbox.appear();
         
         rqjx_showBudget(person_id,org_id,year_now,function(_data) {
            $('innershowbudget').innerHTML = _data;
   ...
   function _idp_view_request_byhrmonitoring($employee_id,$job_id,$back_button=TRUE,$new_button=TRUE,$request_id) {
      require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
      $db=&Database::getInstance();
      $user_id = getUserID();
      require_once(XOCP_DOC_ROOT."/modules/hris/class/ajax_idprequest.php");
