...
function TRN_EX_idp_m_getReportingEventForm($event_id,$employee_id) {
...
function TRN_EX_idp_m_getReportingForm($request_id,$actionplan_id,$preview=0) {
   require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
   $db=&Database::getInstance();
   $user_id = getUserID();
   ...
   $sql = "SELECT a.event_id,a.method_t,b.method_type,a.method_id,c.method_nm,a.plan_start_dttm,a.plan_stop_dttm,a.competency_id"
   ...
   if($event_id>0) {
      ...
      $event_title = "";
      $sql = "DELETE FROM ".XOCP_PREFIX."idp_report_TRN_EX"
   ...
   $sql = "SELECT conclusion,my_advantage,company_advantage,r_01,r_02,r_03,r_04,r_05,r_06,r_07,"
        ...
        . " FROM ".XOCP_PREFIX."idp_report_TRN_EX"
        . " WHERE event_id = '$event_id'"
        . " AND employee_id = '$employee_id'";
   $result = $db->query($sql);
   if($db->getRowsNum($result)==1) {
      ...
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_TRN_EX (event_id,employee_id,actionplan_id,request_id,section_mgr_job_id,section_mgr_emp_id,division_mgr_job_id,division_mgr_emp_id)"
   ...
   $sql = "SELECT idea_id,idea_txt,start_dttm,stop_dttm"
        . " FROM ".XOCP_PREFIX."idp_report_TRN_EX_ideas"
        . " WHERE event_id = '$event_id'"
        . " AND employee_id = '$employee_id'"
        . " ORDER BY idea_id";
   $result = $db->query($sql);
   ...
   $form = "<div style='text-align:center;font-size:1.2em;font-weight:bold;'>TRAINING REPORT</div>"
         ...
         . "<div style='margin-bottom:10px;margin-top:10px;padding:5px;border:1px solid #bbb;-moz-border-radius:5px;'>"
            ...
            . "<tr><td style='font-weight:bold;text-align:right;'>Training Subject : </td><td>$event_title</td><td style='font-weight:bold;text-align:right;'>Institution : </td><td>$institute_nm</td></tr>"
   ...
   require_once(XOCP_DOC_ROOT."/modules/hris/include/idp/ajax/ajax_method_TRN_EX.php");
   $ajax = new _idp_class_method_TRN_EX_ajax("mTRN_EX");
   $js = $ajax->getJs()."<script type='text/javascript'><!--
      ...
      function confirm_delete_idea(idea_id,d,e) {
         ...
         _destroy(tr);
         mTRN_EX_app_deleteIdea('$event_id','$employee_id',idea_id,null);
      }
      
      function new_idea() {
         $('ideaprogress').innerHTML = '';
         $('ideaprogress').appendChild(progress_span());
         mTRN_EX_app_newIdea('$event_id','$employee_id',function(_data) {
            $('ideaprogress').innerHTML = '';
      ...
      function do_approval2(event_id,employee_id) {
         ...
         mTRN_EX_app_approval2Report('$event_id','$employee_id',function(_data) {
      ...
      function do_approval1(event_id,employee_id) {
         $('confirmbtn').innerHTML = '';
         $('confirmbtn').appendChild(progress_span(' ... approve report'));
         mTRN_EX_app_approval1Report('$event_id','$employee_id',function(_data) {
      ...
      function confirm_division_mgr_return_report(event_id,employee_id) {
         ...
         mTRN_EX_app_divisionManagerReturnReport(event_id,employee_id,return_note,function(_data) {
            var data = recjsarray(_data);
      ...
      function confirm_section_mgr_return_report(event_id,employee_id) {
         var return_note = urlencode($('return_note').value);
         mTRN_EX_app_sectionManagerReturnReport(event_id,employee_id,return_note,function(_data) {
            var data = recjsarray(_data);
            location.href = '".XOCP_SERVER_SUBDIR."/index.php?approve=y&request_id='+data[0]+'&actionplan_id='+data[1]+'&r='+uniqid('r');
         });
      }
      ...
      function do_submit(event_id,employee_id) {
         ...
         mTRN_EX_app_submitReport('$event_id','$employee_id',function(_data) {
      ...
      function save_report(d,e) {
         ...
         mTRN_EX_app_saveReport('$event_id','$employee_id',conclusion,my_advantage,company_advantage,rating,idea,function(_data) {
...
function TRN_EX_idp_m_getRemark($request_id,$actionplan_id) {
   $db=&Database::getInstance();
   
   $method_nm = "";
   $institute_nm = "";
   $sql = "SELECT a.event_id,a.method_t,b.method_type,a.method_id,a.competency_id,a.plan_start_dttm,a.plan_stop_dttm,a.cost_estimate,method_subject,a.other_institute_nm FROM ".XOCP_PREFIX."idp_request_actionplan a"
        . " LEFT JOIN ".XOCP_PREFIX."idp_development_method_type b USING(method_t)"
        . " WHERE a.request_id = '$request_id' AND a.actionplan_id = '$actionplan_id'";
   $result = $db->query($sql);
   if($db->getRowsNum($result)==1) {
      list($event_id,$method_t,$method_type,$method_id,$competency_id,$plan_start_dttm,$plan_stop_dttm,$cost_estimate,$method_subject,$other_institute_nm)=$db->fetchRow($result);
      if($event_id>0) {
         ...
         $institute_nm = $other_institute_nm;
      }
   }
   if($event_id>0) {
      $sql = "SELECT status_cd FROM ".XOCP_PREFIX."idp_report_TRN_EX WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
      if($db->getRowsNum($result)==1) {
   ...
   $remark = "<div>$method_nm</div><div style='color:#555;".($institute_nm!=""?"":"display:none;")."'>$institute_nm</div>";
   return array($remark,$plan_start_dttm,$plan_stop_dttm,$report_status,$cost_estimate,$method_nm,$institute_nm);
}


function TRN_EX_idp_m_editActionPlan($request_id,$actionplan_id) {
   $db=&Database::getInstance();
   ...
   if($db->getRowsNum($result)==1) {
      ...
      $dvopthdr = "<div style='padding:3px;text-align:center;background-color:#fff;font-weight:bold;background-color:#ddd;color:black;cursor:default;'>Select External Training</div>"
...
//           . " AND a.rcl_max > '$ccl'"
      ...
      if($db->getRowsNum($result)>0) {
         while(list($method_idx,$method_nmx)=$db->fetchRow($result)) {
            ...
            $dvtemplate .= "<div class='cb' style='padding:3px;border-top:1px solid #bbb;' onclick='_TRN_EX_doselecttemplate(\"$method_idx\",\"$method_nmx\",this,event);'>$method_nmx (subject)</div>";
            ...
            if($db->getRowsNum($revent)>0) {
               while(list($event_idx,$event_title,$event_start_dttm,$event_stop_dttm,$event_cost,$event_registration_t,$institute_nmx)=$db->fetchRow($revent)) {
                  ...
                  $event_cost += 0;
                  $dvevent .= "<div class='cb' style='padding:3px;border-top:1px solid #bbb;' onclick='_TRN_EX_doselectevent(\"$event_idx\",\"$method_idx\",\"$event_title\",\"$event_start_dttm\",\"$event_stop_dttm\",\"$event_cost\",\"$institute_nmx\",this,event);'>"
   ...
   $dvother = "<div class='cb' style='padding:3px;border-top:1px solid #bbb;' onclick='_TRN_EX_doselectother(this,event);'>Other</div>";
   ...
   $ret = "<table id='method_trn_ex_${request_id}_${actionplan_id}' style='width:100%;'>"
               . "<colgroup>"
                  . "<col width='200'/>"
                  . "<col/>"
                  . "<col/>"
        ...
        . "<tr><td style='text-align:right;'>Training Title : </td><td class='cb' colspan='3' style='vertical-align:middle;'>"
            ...
            . "<img class='cb' alt='Click to select' title='Click to select' src='".XOCP_SERVER_SUBDIR."/images/arrow_down.png' style='vertical-align:middle;cursor:pointer;' onclick='_TRN_EX_selecttraining(\"$request_id\",\"$actionplan_id\",this,event);'/>"
            . "<input type='hidden' value='$method_id' id='method_id' name='method_id'/>"
            . "<input type='hidden' value='$event_id' id='event_id' name='event_id'/>"
        . "</td></tr>"
        . "<tr><td style='text-align:right;'>Institute / Provider : </td><td colspan='3'><span id='institute_nm'>$institute_nm</span><input style='width:200px;".($method_id==0&&$method_subject!=""?"":"display:none;")."' type='text' name='other_institute_nm' id='other_institute_nm' value='$other_institute_nm'/></td></tr>"
        . "<tr>"
...
function TRN_EX_idp_m_saveAction($request_id,$actionplan_id,$ret) {
   $db=&Database::getInstance();
   $vars = _parseForm($ret);
...
function _idpm_save(request_id,actionplan_id) {
   ret = _parseForm('method_trn_ex_'+request_id+'_'+actionplan_id);
   save_ap('TRN_EX',request_id,actionplan_id,ret);
}

function _TRN_EX_doselectevent(event_id,method_id,event_title,start_dttm,stop_dttm,cost_estimate,institute_nm,d,e) {
   ...
   $('other_institute_nm').style.display = 'none';
   $('other_institute_nm').value = '';
...
function _TRN_EX_doselecttemplate(method_id,method_nm,d,e) {
...
function _TRN_EX_doselectother(d,e) {
   ...
   $('inpcost').style.display = '';
   $('institute_nm').innerHTML = '';
   $('other_institute_nm').style.display = '';
   $('sp_plan_start_dttm').innerHTML = sql2string($('plan_start_dttm').value);
   $('sp_plan_stop_dttm').innerHTML = sql2string($('plan_stop_dttm').value);
   $('sp_plan_start_dttm').style.display = '';
   $('spstatic_plan_start_dttm').style.display = 'none';
...
function _TRN_EX_selecttraining(request_id,actionplan_id,d,e) {
