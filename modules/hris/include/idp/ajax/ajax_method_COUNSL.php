<?php
//--------------------------------------------------------------------//
// Filename : modules/hris/include/idp/ajax/ajax_method_COUNSL.php     //
...
if ( !defined('HRIS_IDPMETHODCOUNSLAJAX_DEFINED') ) {
   define('HRIS_IDPMETHODCOUNSLAJAX_DEFINED', TRUE);
...
class _idp_class_method_COUNSL_ajax extends AjaxListener {
   
   function _idp_class_method_COUNSL_ajax($act_name) {
      $this->_act_name = $act_name;
      $this->_include_file = XOCP_DOC_ROOT."/modules/hris/include/idp/ajax/ajax_method_COUNSL.php";
      $this->init();
      parent::init();
   }
   ...
   function app_reeditSubject($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL_subject SET "
           . "read_only = '0'"
      ...
      if($db->getRowsNum($result)>0) {
         ...
         if(1) {
            ...
            $sql = "SELECT idea_id,idea_txt,start_dttm,stop_dttm"
                 . " FROM ".XOCP_PREFIX."idp_report_COUNSL_ideas"
                 . " WHERE request_id = '$request_id'"
                 . " AND actionplan_id = '$actionplan_id'"
                 . " AND subject_id = '$subject_id'"
                 . " ORDER BY idea_id";
   ...
   function app_saveReportSubject($args) {
      ...
      if(isset($idea)&&count($idea)>0) {
         $sql = "SELECT idea_id FROM ".XOCP_PREFIX."idp_report_COUNSL_ideas WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
         $result = $db->query($sql);
         if($db->getRowsNum($result)>0) {
            while(list($idea_id)=$db->fetchRow($result)) {
               ...
               $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL_ideas SET "
                    . "idea_txt = '$text',"
   ...
   function app_deleteSubject($args) {
      ...
      $actionplan_id = $args[1];
      $subject_id = $args[2];
      $sql = "DELETE FROM ".XOCP_PREFIX."idp_report_COUNSL_subject"
           . " WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'"
           . " and subject_id = '$subject_id'";
      $db->query($sql);
      $sql = "DELETE FROM ".XOCP_PREFIX."idp_report_COUNSL_ideas"
           . " WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'"
           . " and subject_id = '$subject_id'";
      $db->query($sql);
   }
   ...
   function app_newSubject($args) {
      ...
      $request_id = $args[0];
      $actionplan_id = $args[1];
      $sql = "SELECT MAX(subject_id) FROM ".XOCP_PREFIX."idp_report_COUNSL_subject WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
      $subject_id = 0;
      if($db->getRowsNum($result)>0) {
      ...
      $subject_id++;
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_COUNSL_subject (request_id,actionplan_id,subject_id,subject_dttm) VALUES ('$request_id','$actionplan_id','$subject_id',now())";
      $db->query($sql);
      /*
   ...
   function app_divisionManagerReturnReport($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL SET status_cd = 'approval1', return_note = '$return_note'"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
      $sql = "SELECT approval1_emp_id,request_id,actionplan_id FROM ".XOCP_PREFIX."idp_report_COUNSL"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
   ...
   function app_return1Report($args) {
      ...
      $actionplan_id = $args[1];
      $return_note = addslashes(trim(urldecode($args[2])));
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL SET status_cd = 'prepared', return_note = '$return_note'"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
   ...
   function app_newIdea($args) {
      ...
      $actionplan_id = $args[1];
      $subject_id = $args[2];
      $sql = "SELECT MAX(idea_id) FROM ".XOCP_PREFIX."idp_report_COUNSL_ideas WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id' AND subject_id = '$subject_id'";
      $result = $db->query($sql);
      $idea_id = 0;
      if($db->getRowsNum($result)>0) {
      ...
      $idea_id++;
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_COUNSL_ideas (request_id,actionplan_id,subject_id,idea_id) VALUES ('$request_id','$actionplan_id','$subject_id','$idea_id')";
      $db->query($sql);
      
      $idea_start_dttm = "0000-00-00 00:00:00";
      $idea_stop_dttm = "0000-00-00 00:00:00";
      
      $text = "<input type='text' style='width:95%;' id='textareaidea_${idea_id}_${subject_id}' name='textareaidea_${idea_id}_${subject_id}'/>"
            . "<div style='padding:2px;'><input class='sbtn' type='button' onclick='delete_idea(\"$idea_id\",this,event);' value='Delete Follow Up'/></div>";
      
      $dttm = "<span class='xlnk' id='sp_ideastart_${idea_id}_${subject_id}' onclick='_changedatetime(\"sp_ideastart_${idea_id}_${subject_id}\",\"idea_start_dttm_${idea_id}_${subject_id}\",\"date\",true,false)'>".sql2ind($idea_start_dttm,"date")."</span> - "
            . "<span class='xlnk' id='sp_ideastop_${idea_id}_${subject_id}' onclick='_changedatetime(\"sp_ideastop_${idea_id}_${subject_id}\",\"idea_stop_dttm_${idea_id}_${subject_id}\",\"date\",true,false);'>".sql2ind($idea_stop_dttm,"date")."</span>"
            . "<input type='hidden' id='idea_start_dttm_${idea_id}_${subject_id}' name='idea_start_dttm_${idea_id}_${subject_id}' value='$idea_start_dttm'/>"
            . "<input type='hidden' id='idea_stop_dttm_${idea_id}_${subject_id}' name='idea_stop_dttm_${idea_id}_${subject_id}' value='$idea_stop_dttm'/>";
      
      return array($idea_id,$subject_id,$text,$dttm);
   }
   ...
   function app_approval2Report($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL SET status_cd = 'completed', approval2_dttm = now()"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
      
      $sql = "SELECT request_id,actionplan_id FROM ".XOCP_PREFIX."idp_report_COUNSL"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
   ...
   function app_approval1Report($args) {
      ...
      $sql = "SELECT approval2_emp_id FROM ".XOCP_PREFIX."idp_report_COUNSL"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
      ...
      if($doubleapproval==1) {
         $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL SET status_cd = 'approval2', approval1_dttm = now(), return_note = ''"
              . " WHERE request_id = '$request_id'"
              . " AND actionplan_id = '$actionplan_id'";
         $db->query($sql);
         ...
         $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL SET status_cd = 'completed', approval1_dttm = now(), return_note = ''"
              . " WHERE request_id = '$request_id'"
              . " AND actionplan_id = '$actionplan_id'";
         $db->query($sql);
   ...
   function app_submitReport($args) {
      ...
      $request_id = $args[0];
      $actionplan_id = $args[1];
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL SET return_note = '', status_cd = 'approval1', submit_dttm = now()"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
      $sql = "SELECT approval1_emp_id FROM ".XOCP_PREFIX."idp_report_COUNSL"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
   ...
   function app_saveReport($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL SET "
           . "r_01 = '$r_01',"
           . "r_02 = '$r_02',"
      ...
      if(isset($idea)&&count($idea)>0) {
         $sql = "SELECT subject_id,idea_id FROM ".XOCP_PREFIX."idp_report_COUNSL_ideas WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
         $result = $db->query($sql);
         if($db->getRowsNum($result)>0) {
            while(list($subject_id,$idea_id)=$db->fetchRow($result)) {
               $text = addslashes(trim($idea["textareaidea_${idea_id}_${subject_id}"]));
               $start_dttm = $idea["idea_start_dttm_${idea_id}_${subject_id}"];
               $stop_dttm = $idea["idea_stop_dttm_${idea_id}_${subject_id}"];
               $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL_ideas SET "
      ...
      if(isset($subject)&&count($subject)>0) {
         $sql = "SELECT subject_id FROM ".XOCP_PREFIX."idp_report_COUNSL_subject WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
         $result = $db->query($sql);
         if($db->getRowsNum($result)>0) {
            while(list($subject_id)=$db->fetchRow($result)) {
               $text = addslashes(trim($subject["textareasubject_${subject_id}"]));
               $sql = "UPDATE ".XOCP_PREFIX."idp_report_COUNSL_subject SET "
                    . "subject_txt = '$text'"
                    . " WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'"
                    . " and subject_id = '$subject_id'";
...
} /// HRIS_IDPMETHODCOUNSLAJAX_DEFINED
?>