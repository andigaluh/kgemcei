...
class _hris_CompiledReport extends XocpBlock {
   ...
   function listSession() {
      ...
      $tbl = "<table class='xxlist'><thead><tr>"
           ...
           . "<td style='text-align:center;'>JM<br/>Total</td>"
           . "<td style='text-align:center;'>JM<br/>General</td>"
           . "<td style='text-align:center;'>JM<br/>Managerial</td>"
           . "<td style='text-align:center;'>KPI</td>"
           . "<td style='text-align:center;'>Action Plan</td>"
      ...
      foreach($jobs as $job_idx=>$job) {
         ...
         if($db->getRowsNum($remp)>0) {
            if($job_class_nm_old!=$job_class_nm) {
               $tbl .= "<tr><td colspan='10' style='font-weight:bold;text-align:left;background-color:#ddd;'>$job_class_nm</td></tr>";
            ...
            while(list($employee_id,$nip,$employee_nm)=$db->fetchRow($remp)) {
               ...
               $sql = "SELECT a.cclxxx,b.compgroup_id,c.rcl,c.itj"
                    . " FROM ".XOCP_PREFIX."employee_competency_final a"
                    . " LEFT JOIN ".XOCP_PREFIX."competency b USING(competency_id)"
                    . " LEFT JOIN ".XOCP_PREFIX."assessment_session_job_competency c ON c.asid = a.asid AND c.job_id = a.job_id AND c.competency_id = a.competency_id"
                    . " WHERE a.asid = '$asid'"
               ...
               if($db->getRowsNum($rc)>0) {
                  while(list($cclxxx,$compgroup_id,$rcl,$itj)=$db->fetchRow($rc)) {
                     if($compgroup_id==1) {
                        $ttlccl_general += ($cclxxx*$itj);
                        $ttlrcl_general += ($rcl*$itj);
                     } else if ($compgroup_id==2) {
                        $ttlccl_managerial += ($cclxxx*$itj);
                        $ttlrcl_managerial += ($rcl*$itj);
                     }
                  }
                  $jm_general = 100*$ttlccl_general/$ttlrcl_general;
                  $jm_managerial = 100*$ttlccl_managerial/$ttlrcl_managerial;
               }
               ...
               $tbl .= "<tr><td>$nip</td><td>$employee_nm</td><td>$job_abbr</td><td>$org_abbr</td>"
                     ...
                     . "<td style='text-align:center;'>".toMoney($jm_general)."</td>"
                     . "<td style='text-align:center;'>".toMoney($jm_managerial)."</td>";
               
               
               $sql = "SELECT a.reported_final_result,a.kpi_final_result,a.ap_final_result"
                    . " FROM pms_jam a"
               ...
               $result = $db->query($sql);
               $ttl_achievement = 0;
               $kpi_ttl_achievement = 0;
               $ap_ttl_achievement = 0;
               if($db->getRowsNum($result)>0) {
                  while(list($reported_final_result,$kpi_final_result,$ap_final_result)=$db->fetchRow($result)) {
                     $ttl_achievement = _bctrim(bcadd($ttl_achievement,$reported_final_result));
                     $kpi_ttl_achievement = _bctrim(bcadd($kpi_ttl_achievement,$kpi_final_result));
                     $ap_ttl_achievement = _bctrim(bcadd($ap_ttl_achievement,$ap_final_result));
                  }
               }
               
               $tbl .= "<td style='text-align:center;'>".toMoney($kpi_ttl_achievement)."</td>";
               $tbl .= "<td style='text-align:center;'>".toMoney($ap_ttl_achievement)."</td>";
