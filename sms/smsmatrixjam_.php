   ...
   function smsmatrixjam() {
	...
	$ret .= " <tr height='75'>"
						...
						."<td style='vertical-align:top;border-right:1px solid #bbb;text-align:center;color:blue;font-weight:bold;' rowspan='2'></td>"
							."<td style='vertical-align:top;border-right:1px solid #bbb;'>"
								."<span style='color:black;font-weight:bold;'>$objective_title </span>";
				$sqlactionplan = "SELECT DISTINCT a.sms_action_plan_id, a.sms_action_plan_text,a.sms_kpi_id FROM sms_action_plan a JOIN sms_kpi b ON ( a.sms_objective_id = b.sms_objective_id ) WHERE b.sms_objective_id = $objid";
				...
				$sqlactionplan = "SELECT DISTINCT a.sms_action_plan_id, a.ap_lower_perform,a.sms_kpi_id FROM sms_action_plan a JOIN sms_kpi b ON ( a.sms_objective_id = b.sms_objective_id ) WHERE b.sms_objective_id = $objid";
				$resultactionplan = $db->query($sqlactionplan);
			
					$ret .= "<table><br>";
					
					while(list($id_ap,$ap_lower_perform,$kpid)=$db->fetchRow($resultactionplan)){
							if($ap_lower_perform == '')
				...
				$sqlactionplan = "SELECT DISTINCT a.sms_action_plan_id,a.ap_need_improvement,a.sms_kpi_id FROM sms_action_plan a JOIN sms_kpi b ON ( a.sms_objective_id = b.sms_objective_id ) WHERE b.sms_objective_id = $objid";
				$resultactionplan = $db->query($sqlactionplan);

					$ret .= "<table><br>";
					
					while(list($id_ap,$ap_need_improvement,$kpid)=$db->fetchRow($resultactionplan)){
				...
				$sqlactionplan = "SELECT DISTINCT b.sms_kpi_id,b.sms_kpi_target_text,a.sms_kpi_id FROM sms_action_plan a JOIN sms_kpi b ON ( a.sms_objective_id = b.sms_objective_id ) WHERE b.sms_objective_id = $objid";
				$resultactionplan = $db->query($sqlactionplan);
			
					$ret .= "<table><br>";
					
					while(list($sms_kpi_id,$sms_kpi_target_text,$kpid)=$db->fetchRow($resultactionplan)){
				...
				$sqlactionplan = "SELECT DISTINCT a.sms_action_plan_id,a.ap_req,a.sms_kpi_id FROM sms_action_plan a JOIN sms_kpi b ON ( a.sms_objective_id = b.sms_objective_id ) WHERE b.sms_objective_id = $objid";
				$resultactionplan = $db->query($sqlactionplan);
			
					$ret .= "<table><br>";
					
					while(list($id_ap,$ap_req,$kpid)=$db->fetchRow($resultactionplan)){
							if($ap_req == '')
							{ $ap_req = 'Empty';	}
							else
							{	$ap_req = $ap_req; }
				...
				$sqlactionplan = "SELECT DISTINCT a.sms_action_plan_id,a.ap_far_req,a.sms_kpi_id FROM sms_action_plan a JOIN sms_kpi b ON ( a.sms_objective_id = b.sms_objective_id ) WHERE b.sms_objective_id = $objid";
				$resultactionplan = $db->query($sqlactionplan);
			
					$ret .= "<table><br>";
					
					while(list($id_ap,$ap_far_req,$kpid)=$db->fetchRow($resultactionplan)){
							if($ap_far_req == '')
							{ $ap_far_req = 'Empty';	}
							else
				...
				$wgt5 = $objective_weight * 100/100;
					...
					$ret	.= "</tr>";
      $ret .= "<tr>"
					."<td style='border-right:1px solid #bbb;text-align:right;'><span class='xlnk' onclick='edit_target_text(\"$objid\",11,this,event);'>$objective_weight</span> %</td>"
	...
	$sqlkpi = "SELECT SUM(DISTINCT a.weight) 
		FROM sms_section_objective a
		LEFT JOIN sms_session b ON ( a.id_section_session = b.id )
		LEFT JOIN sms_kpi d ON ( a.id = d.sms_objective_id )
		LEFT JOIN sms_action_plan c ON ( c.sms_objective_id = d.sms_objective_id )
