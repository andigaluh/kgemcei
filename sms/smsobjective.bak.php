...
class _sms_Objective extends XocpBlock {
   ...
   function showOrg($showOpt=FALSE) {
      ...
      $sql = "SELECT a.employee_id,e.org_id,e.org_nm,f.org_class_id"
       ...
       . " WHERE a.status_cd = 'normal' AND a.person_id = $person_id AND c.gradeval > '9' OR a.person_id = '1'";// 
      $result = $db->query($sql);
      ...
      //if($db->getRowsNum($result)>0) {
         //while(list($sms_perspective_code,$sms_perspective_id,$sms_perspective_name)=$db->fetchRow($result)) {
            $ret .= "<tr><td style='border:0px;border-bottom:0px solid #333;background-color:#f1f1f1;display:none;' colspan='".(6+$picnum+($share_cnt==0?0:$share_cnt))."'>&nbsp;</td></tr>"
                  ...
                  //. "$sms_perspective_name Perspective"
                  . ($current_org_class_id == 4 ?"&nbsp;&nbsp;<span class='xlnk' onclick='edit_so(\"new_${sms_perspective_id}\",this,event);' style='font-weight:normal;'/><img src='".XOCP_SERVER_SUBDIR."/images/add_behavior.png' style='width:17px;float:left;'>Add Objective</span>&nbsp;":"")
                  //. "&nbsp;&nbsp;[<span class='xlnk' onclick='edit_so(\"new_${sms_perspective_id}\",this,event);' style='font-weight:normal;'/>Add Objective</span>]&nbsp;"
                  . "</td>"
