              ...
              . "<div style='max-height:280px;height:280px;overflow:auto;border:1px solid #999;background-color:#fff;padding:4px;' id='frmobj'>"
				...
				$ret .= "<select name='psid' id='psid'>";
						while(list($psid,$periode)=$db->fetchRow($resultsess)) {
								$ret.= "<option id='psid' value='$psid'>$periode</option>";
								}    
...
//	BUAT RETURN YANG APPROVAL;
	  ...
	  $sql =  "SELECT id,id_session,employee_id FROM sms_jam_approval "
			  . "  WHERE id_session = '$psid' AND  employee_id = '$person_id' " ;
	  $result = $db->query($sql); 
    	 
       
	   list($id,$psid,$person_id)=$db->fetchRow($result);
      ...
      $ret = "<form id='frm' style='padding-top:10px;'><table class='xxfrm' style='width:100%;'><colgroup><col width='160'/><col/></colgroup><tbody>"
           . "<tr><td>Return Note</td><td><input type='text' value='' id='inp_note' name='inp_note' style='width:400px;'/></td></tr>" ;
	...
	function app_saveReturn($args) {
      ...
      $vars = parseForm($args[1]);
      //$inp_note = _bctrim(bcadd(0,$vars["inp_note"]));


	  $sql = "UPDATE sms_jam_approval SET  propose_stat = '0', propose_date = '0000-00-00', approve1_by = ' ', approve1_stat = '0', approve1_date = '0000-00-00', approve2_by = ' ',  approve2_stat = '0', approve2_date = '0000-00-00' , status = 'return', date_return = '$date_modified' WHERE id_session = '$psid' AND  employee_id = '$person_id'";
	  $db->query($sql);
