...
class _sms_class_SMSObjAjax extends AjaxListener {
   ...
   function init() {
      $this->registerAction($this->_act_name,"app_Delete","app_editSession","app_saveSession",
                                             "app_setANTRAINSession","app_newSession","app_saveObjTargetText","app_addObjOwner","app_saveObjectiveOwner","app_addMeasure","app_saveMeasure","app_newKsf","app_saveKsf","app_editKsf","app_deleteKsf","app_editPicObj","app_deletePicObj","app_savePicObj","app_confirmdeleteMeasure","app_deleteMeasure","app_deleteObjOpt","app_deleteObj","app_appropose","app_apapprove","app_setSessions");
   }
    ...
    function app_setSessions($args) {
	  ...
	  $_SESSION["id_temps"] = $id;
	  $sesid =  $_SESSION["id_temps"] ;
      return array( $sesid);
  ...
  function app_saveObjTargetText($args) {
	...
	elseif ($no == 4) {
	  $sql = "UPDATE sms_objective_measure SET measure_description = '$val' WHERE id = '$ids'";
       $db->query($sql); }
	elseif ($no == 41) {
	  $sql = "UPDATE sms_objective_measure SET result = '$val' WHERE id = '$ids'";
       $db->query($sql); }
	elseif ($no == 42) {
	  $sql = "UPDATE sms_objective_measure SET percentage = '$val' WHERE id = '$ids'";
       $db->query($sql); }
	elseif ($no == 43) {
	  $sql = "UPDATE sms_objective_measure SET remark = '$val' WHERE id = '$ids'";
       $db->query($sql); }
...
#DELETE MEASURE
    ...
    function app_confirmdeleteMeasure($args) {
      $db=&Database::getInstance();
	  $id_del = $args[0];
	  $id_measure = $args[1];
	...
	#DELETE Measure	 
	 if( $id_del==1){
			  $sql = "DELETE FROM sms_objective_measure WHERE id = '$id_measure'";
          $db->query($sql);
		  
		  return array($id_del,$id_measure);
     }
	 
	 #DELETE intent	 
	 elseif($id_del==2)
	 {
		$sql = "DELETE FROM sms_objective_intent WHERE id = '$id_measure'";
        $db->query($sql);
	 ...
	 elseif($id_del==3)
	 {
		  $sql = "DELETE FROM sms_objective_frequency WHERE id = '$id_measure'";
          $db->query($sql);
		  return array($id_del,$id_measure);
		}
	...
	elseif($id_del==4)
	 {
		  $sql = "DELETE FROM sms_objective_target WHERE id = '$id_measure'";
          $db->query($sql);
	...
	elseif($id_del==10)
	 {
		  $sql = "DELETE FROM sms_measure_attach WHERE id = '$id_measure'";
          $db->query($sql);
   ...
   function app_deleteMeasure($args) {
      $db=&Database::getInstance();
      $id_del = $args[0];
	  $id_measure = $args[1];
    ...
    if( $id_del==1)
	{
	$btn = "<input type='button' value='"._DELETE."' onclick='confirmdelete_measure(\"$id_del\",\"$id_measure\");'/>&nbsp;&nbsp;"
              . "&nbsp;<input type='button' value='"._CANCEL."' onclick='persbox.fade();'/>";
 
    $date =getSQLDate();

	$sqlmeasure = "SELECT id,measure_code,measure_description FROM sms_objective_measure WHERE id = $id_measure";
	$resultmeasure = $db->query($sqlmeasure);
	list($idmes,$measure_code,$measure_description)=$db->fetchRow($resultmeasure);
	
	$ret = "<div style='height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;'>"
           . "Delete Measure"
           . "</div>"
           . "<div style='padding:5px;background-color:#f2f2f2;color:#555;'>"
              . "<div style='max-height:40px;height:40px;overflow:auto; solid #999;background-color:#fff;padding:4px;' id='frmobj'>"
                  . "<table style='width:100%;border-spacing:0px;'><tbody><tr><td style='height:40px;'>"
                     . "<table class='xxfrm' style='width:100%;'><tbody>"     
						. "<tr>Delete Measure $measure_code - $measure_description ? </tr>"
    ...
    $date =getSQLDate();
	 ...
	 $sqlintent = "SELECT id,intent_code,intent_description FROM sms_objective_intent WHERE id = $id_measure";
	 $resultintent = $db->query($sqlintent);
	 list($idint,$intent_code,$intent_description)=$db->fetchRow($resultintent);
	
	$ret = "<div style='height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;'>"
           . "Delete Measure Intent"
           . "</div>"
           . "<div style='padding:5px;background-color:#f2f2f2;color:#555;'>"
              . "<div style='max-height:40px;height:40px;overflow:auto; solid #999;background-color:#fff;padding:4px;' id='frmobj'>"
                  . "<table style='width:100%;border-spacing:0px;'><tbody><tr><td style='height:40px;'>"
                     . "<table class='xxfrm' style='width:100%;'><tbody>"     
						. "<tr>Delete Measure Intent $intent_code - $intent_description ? </tr>"
    ...
    $date =getSQLDate();
	 ...
	 $sqlfrequency = "SELECT id,frequency_code,frequency_description FROM sms_objective_frequency WHERE id = $id_measure";
	 $resultfrequency = $db->query($sqlfrequency);
	list($idfreq,$frequency_code,$frequency_description)=$db->fetchRow($resultfrequency);
	
	
	
	
	$ret = "<div style='height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;'>"
           . "Delete Measure Frequency"
           . "</div>"
           . "<div style='padding:5px;background-color:#f2f2f2;color:#555;'>"
              . "<div style='max-height:40px;height:40px;overflow:auto; solid #999;background-color:#fff;padding:4px;' id='frmobj'>"
                  . "<table style='width:100%;border-spacing:0px;'><tbody><tr><td style='height:40px;'>"
                     . "<table class='xxfrm' style='width:100%;'><tbody>"     
						. "<tr>Delete Measure $frequency_code - $frequency_description ? </tr>"
    ...
    $date =getSQLDate();
	 ...
	 $sqltarget = "SELECT id,target_code,target_high,target_medium,target_low FROM sms_objective_target WHERE id = $id_measure";
	 $resulttarget = $db->query($sqltarget);
	 list($idtgt,$target_code,$target_high,$target_medium,$target_low)=$db->fetchRow($resulttarget);
	...
	$ret = "<div style='height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;'>"
           ...
           . "<div style='padding:5px;background-color:#f2f2f2;color:#555;'>"
              . "<div style='max-height:40px;height:40px;overflow:auto; solid #999;background-color:#fff;padding:4px;' id='frmobj'>"
                  . "<table style='width:100%;border-spacing:0px;'><tbody><tr><td style='height:40px;'>"
                     . "<table class='xxfrm' style='width:100%;'><tbody>"     
						. "<tr>Delete Measure $target_code - $target_high,$target_medium,$target_low ? </tr>"
    ...
    $date =getSQLDate();
	 ...
	 $sqlatt = "SELECT id,attach FROM sms_measure_attach WHERE id = $id_measure";
	 $resultatt = $db->query($sqlatt);
	 list($idatt,$attachment)=$db->fetchRow($resultatt);
	...
	$ret = "<div style='height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;'>"
           ...
           . "<div style='padding:5px;background-color:#f2f2f2;color:#555;'>"
              . "<div style='max-height:40px;height:40px;overflow:auto; solid #999;background-color:#fff;padding:4px;' id='frmobj'>"
                  . "<table style='width:100%;border-spacing:0px;'><tbody><tr><td style='height:40px;'>"
                     . "<table class='xxfrm' style='width:100%;'><tbody>"     
						. "<tr>Delete Attachment $attachment ? </tr>"
           ...
           . "</div>"
           . "<div style='text-align:center;padding:10px;background-color:#f2f2f2;height:100px;' id='frmbtn'>";
      $ret .=  "<input type='button' value='"._DELETE."' onclick='confirmdelete_measure(\"$id_del\",\"$idatt\");'/>&nbsp;&nbsp;"
              . "&nbsp;<input type='button' value='"._CANCEL."' onclick='persbox.fade();'/>"
            . "</div>";
    ...
    function app_newKsf($args) {
     ...
     function app_saveKsf($args) {
        ...
        if($id=="new") {
           ...
           $create_user_id = getUserID();
           $create_date = getSQLDate();
           //$sql = "INSERT INTO sms_objective_ksf (id,id_objective,ksf_code,ksf_title,ksf_target,create_user_id,create_date) VALUES('$id','$idobj','$code_ksf','$title_ksf','$create_user_id','$create_date')";
           $sql = "INSERT INTO sms_objective_ksf (id,id_objective,ksf_code,ksf_title,ksf_target,create_user_id,create_date)"
             . " VALUES('$id','$idobj','$code_ksf','$title_ksf','$target_ksf','$create_user_id','$create_date')";
           $db->query($sql);
        } else {
   ...
   function app_deleteObjOpt($args) {
   	$db=&Database::getInstance();
   	$idobj = $args[0];

   	$ret = "<div style='height:21px;padding-top:7px;text-align:center;background-color:#6d84b4;font-weight:bold;color:#eee;font-size:1.1em;'>Delete Objective Confirmation"
           . "</div>"
           . "<div style='height:90px;padding:5px;background-color:#f2f2f2;color:#555;'>"
           . "<div  style='text-align:center;padding-top:30px;'>"
           . "<span>Are you going to delete this objective</span>"
           . "</div>"
           . "</div>"
           . "<div style='text-align:center;padding:10px;background-color:#f2f2f2;' id='frmbtn'>"
           . "<input type='button' onclick='do_delete_obj(\"$idobj\",this,event)' value='Yes' style='width:50px;' />"
           . "<input type='button' onclick='objbox.fade();' value='No' style='width:50px;margin-left:10px;' />"
           . "</div>";

   	return $ret;
   }

   function app_deleteObj($args) {
   	$db=&Database::getInstance();
   	$idobj = $args[0];

   	$sql = "UPDATE sms_objective SET status = 'null' WHERE id = '$idobj'";
   	$db->query($sql);
   }
   ...
   //update fahmi actionplan propose & approval
	function app_appropose($args){
	$db=&Database::getInstance();
   	$propose_id = getpersonId();
	$propose_date = getSQLDate();
	$prop_dt = date('d/M/Y', strtotime($propose_date));
	$sql = "INSERT INTO sms_ap_approval (propose_id,propose_date,obj_id,per_id) VALUES ('$propose_id','$propose_date','$prop_dt','$idobj',$idper)'";
    $db->query($sql);
