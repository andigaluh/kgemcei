<?php
//--------------------------------------------------------------------//
// Filename : modules/sms/class/selectsession.php                         //
...
class _sms_class_SelectSession extends XocpAttachable {
   ...
   function showSmsSession($showOpt=FALSE) {
	...
	$sql = "SELECT id,periode_session,title_session"
      ...
      $sessionopt = "";
      if($cnt == 1) {
       list($psid,$year,$title_session) = $db->fetchRow($result);
		 
	     $_SESSION["antrain_year"] = $year;
         $_SESSION["pms_psid"] = $psid;
         $sessionopt .= "<div style='padding:3px;'><span onclick='_antrain_select_antrain(\"$psid\",this,event);' class='xlnk'>$year $title_session </span></div>";
      } 
	  else if($cnt > 1)
	  {
			$found = 0;
		 
			while(list($psid,$year,$title_session)=$db->fetchRow($result)) {
      ...
      return $js."<div class='orgsel'><table border='0' width='100%' cellpadding='2' cellspacing='0'>
              ...
              <td align='right'>[<span class='xlnk' id='chorgsp' onclick='return show_session_opt(this,event);'>Select Session"
