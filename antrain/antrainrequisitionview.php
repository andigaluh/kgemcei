...
include_once(XOCP_DOC_ROOT."/modules/antrain/class/selectantrainreqview.php");

class _antrain_Requisitionview extends XocpBlock {
   var $catchvar = _ANTRAIN_CATCH_VAR;
   var $blockID = _ANTRAIN_REQUISITIONVIEW_BLOCK;
   var $width = "100%";
   var $language;
   ...
   function _antrain_Requisitionview($catch=NULL) { /* fungsi konstruktor wajib punya parameter $catch
   ...
   function antrainrequisitionview() {
      ...
      $result = $db->query($sql);
      list($year,$is_proposed,$is_proposeddiv,$is_approved,$is_approvedfgm,$date_proposed,$org_nm,$org_abbr)=$db->fetchRow($result);
...
//PROPOSE
	if ( $job_id == 0 || $job_id == 999|| $job_id == 141|| $job_id == 68|| $job_id == 138|| $job_id == 132|| $job_id == 151|| $job_id == 37|| $job_id == 169|| $job_id == 107|| $job_id == 111|| $job_id == 74|| $job_id == 85|| $job_id == 49|| $job_id == 60|| $job_id == 91|| $job_id == 183|| $job_id == 25|| $job_id == 211|| $job_id == 220 ) {
	 ...
	 //Return to SM
	 if( $job_id == 0 || $job_id == 999|| $job_id == 6|| $job_id == 36|| $job_id == 48|| $job_id == 104|| $job_id == 126|| $job_id == 147|| $job_id == 168 AND $is_proposed == 1)
	 {
	 ...
	 if( $job_id == 0 || $job_id == 999|| $job_id == 47|| $job_id == 146|| $job_id == 167|| $job_id == 129 AND $is_proposeddiv == 1)
	 {
	 ...
	 //PROPOSEDIV
	  if ($job_id == 0 || $job_id == 999|| $job_id == 6|| $job_id == 36|| $job_id == 48|| $job_id == 104|| $job_id == 126|| $job_id == 147|| $job_id == 168 || $job_id == 146 )
...
//APPROVEDFGM  
	...
	if ( $job_id == 999|| $job_id == 0|| $job_id == 129 )
	   {
			if($is_approvedfgm == 1 || $is_approved == 0 )
			{
			$approvedfgmbutton = '';
			}
			else
...
FROM antrain_requisition p LEFT JOIN antrain_sessionreq pk ON p.id_antrain_session = pk.psid LEFT JOIN hris_job_class ps ON p.id_job_class1 = ps.job_class_id LEFT JOIN hris_job_class ps2 ON p.id_job_class2 = ps2.job_class_id WHERE p.id_antrain_session = '$psid' AND p.is_deleted = 'F' AND p.employee_id = '$employee_id'";
	$result = $db->query($sql);
	...
	$sql = "SELECT SUM(cost) FROM antrain_requisition WHERE id_antrain_session = '$psid' AND is_deleted = 'F' AND employee_id = '$employee_id'";
	$result = $db->query($sql);
	...
	$ret .= "<tr style='display:none;' id='tremptyses'><td>"._EMPTY."</td></tr>"; 
	 ...
	 $form .=   "<table align='right' style='border-top:2px solid #777;border-left:2px solid #777;border-spacing:0px;margin-top:25px;'>"
                . "<colgroup>"
                . "<col width='155'/>"
				. "<col width='155'/>"
                . "<col width='155'/>"
...
//SESSION DELETE ASK
      ...
      function save_session() {
		...
		//$('td_objectives_'+data[0]).innerHTML = data[3];
			...
			$('td_cost_'+data[0]).innerHTML = 'Rp. '+data[6];
 ...
 var wdv = null;
   ...
   function main() {
      ...
      switch ($this->catch) {
         case $this->blockID:
            $ret = $this->antrainrequisitionview($self_employee_id);
            break;
         default:
            $ret = $this->antrainrequisitionview($self_employee_id);
            break;
