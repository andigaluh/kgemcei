...
if ( !defined('ANTRAIN_ANTRAINSESSIONSS_DEFINED') ) {
   define('ANTRAIN_ANTRAINSESSIONSS_DEFINED', TRUE);

include_once(XOCP_DOC_ROOT."/modules/antrain/modconsts.php");

class _antrain_ANTRAINSessionss extends XocpBlock {
   var $catchvar = _HRIS_CATCH_VAR;
   var $blockID = _ANTRAIN_ANTRAINSESSIONSS_BLOCK;
   var $width = "100%";
   var $language;
   var $display_title = TRUE;
   var $title = 'Annual Training Session - Specific Subject';
   ...
   function listSession() {
      ...
      require_once(XOCP_DOC_ROOT."/modules/antrain/class/ajax_antrainsessionss.php");
      $ajax = new _antrain_class_ANTRAINSessionSSAjax("psjx");
	...
	$resultjob = $db->query($sqljob);
	 ...
	 list($job_nm,$job_class_nm,$org_nm,$org_class_nm,$org_ids)=$db->fetchRow($resultorg);
		...
		elseif($org_class_nm == 'Division')
			{
				$divsecid = " AND pk.org_id = '$org_id_sec' ORDER BY year";
			}
		//edit disini untuk filter by Section
		elseif($org_class_nm == 'Section')
			 {
				$divsecid = " AND p.org_id_sec = '$org_id_sec' ORDER BY year";
			 }
	  ...
	  $sql = "SELECT p.psid,  p.year,p.org_id, p.budget, p.remark, pk.org_nm FROM antrain_sessionss p LEFT JOIN  hris_orgs pk ON p.org_id = pk.org_id WHERE p.status_cd = 'normal' $divsecid";	   
      $result = $db->query($sql);
	 ...
	 $ret = "<table class='xxlist' style='width:100%;' align='center'>"
      ...
      if($db->getRowsNum($result)>0) {
         while(list($psid,$year,$org_id,$budget,$remark,$org_nm)=$db->fetchRow($result)) {
			...
			$sqlsec = "SELECT pk.org_nm FROM antrain_sessionss p LEFT JOIN hris_orgs pk ON p.org_id_sec = pk.org_id WHERE p.status_cd =  'normal' AND p.psid = '$psid' ORDER BY YEAR DESC ";	   
