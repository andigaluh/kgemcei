...
include_once(XOCP_DOC_ROOT."/modules/antrain/class/selectantrainapproval.php");

class _antrain_Plan_approval extends XocpBlock {
   var $catchvar = _ANTRAIN_CATCH_VAR;
   var $blockID = _ANTRAIN_PLAN_BLOCK;
   var $width = "100%";
   var $language;
   var $display_title = TRUE;
   var $title = 'Annual Training Plan - General Subject Approval';
   ...
   function antrainplanapproval() {
      ...
      $sql = "SELECT p.year, p.is_approved, p.date_approved,  pk.org_nm, pk.org_class_id FROM antrain_session p LEFT JOIN hris_orgs pk ON p.org_id = pk.org_id WHERE psid = ('$psid') ORDER BY year DESC " ;
	  
	  /* "SELECT year,session_nm FROM antrain_session WHERE psid  = '$psid'"; */
      $result = $db->query($sql);
      list($year,$is_approved,$date_approved,$org_nm)=$db->fetchRow($result);
   $approvedbutton = "<form id='approvedbutton' method= 'post'><input type='hidden' name='id_approved' value='$id_approvee' ><input type='hidden' name='is_approved' value='$is_approve' ><input type='hidden' name='date_approved' value='$dt_approve' ><input type='button' onclick='approve();' value='Approve'  id='apprbut'></form>";
	   $returnbtn = "<span style='float:right; margin:5px;'><input onclick='return_sessionapproval(\"$psid\",this,event);' type='button' value='Return' id='returnbtn'/></span>";   
	if ($sumcost == 0)
	{
		$sumcost = '0' ;
	}
	...
	$tooltip = "<div id='all_ap_tooltip' style='display:none;'>";
     ...
     /*  $ret .= $return_notes;
      ...
      $ret .= "<div style='padding:5px;font-size:20px;color: #666666; text-align:center;'>ANNUAL TRAINING / SEMINAR PLAN</div>"
			 ...
			 . "<div style='padding:2px;font-size:12px;color: #666666; text-align:left;'>Division/ Section:  $org_nm $returnbtn<div id='id_return'></div></div>"
			 //. "<span style='float:right; margin:5px;'><input onclick='new_session();' type='button' value='Add'/></span>"
			...
			. "<tr><td style='border-right: 1px solid #CCC;  text-align:center;' rowspan='2'>Participant Target</td><td style='border-right: 1px solid #CCC; text-align:center;' rowspan='2'>Number</td><td style='border-right: 1px solid #CCC;  text-align:center;' rowspan='2'>Subject</td><td style='border-right: 1px solid #CCC;  text-align:center;' rowspan='2'>Objective</td><td style='border-right: 1px solid #CCC; text-align:center;' rowspan='2'>Schedule</td><td style='border-right: 1px solid #CCC; text-align:center;' rowspan='2'>Cost Estimation</td><td style='border-right: 1px solid #CCC; text-align:center;' colspan='2'>Instructor*</td></tr>"
			. "<tr><td style='border-right: 1px solid #CCC; text-align:center;'>Int.</td><td style='border-right: 1px solid #CCC; text-align:center;'>Ext.</td></tr></thead>";
		 ...
		 /*  $ret .= "<tr id='tdclass_${id}'  style='margin: 0px;'>"
			...
			//edit by andi
			  $ret .= "<tr><td id='tdclass_${id}' style='margin: 0px; padding:0px;' colspan=10 >"
					. "<table style='width:100%'><colgroup><col width='3%'/><col width='17%'/><col width='5%'/><col width='12%'/><col width='12%'/><col width='12%'/><col width='12%'/><col width='7%'/><col width='7%'/><col width='10%'/></colgroup><tbody><tr>"
				  . "<td id='td_num_loop_${id}' style='text-align:center;width:50px'>$numberloop</td>"
				 . "<td><span id='sp_${id}'>".$participant ."</span></td>"
               	  . "<td id='td_number_${id}' style='text-align: center;'>$number</td>"
				  . "<td id='td_subject_${id}' style='text-align: left;'>$subject</td>"
				  . "<td id='td_objectives_${id}' style='text-align: left;'>$objectives</td>"
   ...
   $form .= "<div style='text-align:right;padding:10px;margin-top:10px;margin-bottom:100px;'>"
      ...
      function save_session() {
		...
		orgjx_app_saveSession (wdv.id,ret,function(_data) {
	  	  ...
	  	  var wdv = null;
      function return_sessionapproval(psid,d,e) {
         if(wdv) {
            if(wdv.psid == psid) {
               cancel_return();
               return;
        ...
        // wdv.appendChild(progress_span());
         // wdv.td = td;
         orgjx_app_returnSessionApproval(psid,function(_data) {
           // wdv.innerHTML = _data;
		    $('id_return').innerHTML = _data;
           $('inp_year').focus();
         });
	  ...
	  function save_returnapproval(id) {
         var ret = parseForm('frm');
         $('progress').appendChild(progress_span('&nbsp;...saving&nbsp;&nbsp;'));
         orgjx_app_saveReturnApproval(id,ret,function(_data) {
            var data = recjsarray(_data);
            // $('sp_'+data[0]).innerHTML = data[1];
			// $('td_remark_'+data[0]).innerHTML = data[2];
            // $('inp_year').focus();
   ...
   function main() {
      ...
      switch ($this->catch) {
         case $this->blockID:
            $ret = $this->antrainplanapproval($self_employee_id);
            break;
         default:
            $ret = $this->antrainplanapproval($self_employee_id);
            break;
	...
	//echo 'getuserid = '.getuserid(). '<br> job_id = '.$job_id; exit();
	 	 if ($job_id == 90 || $job_id == 68 || $job_id == 101  || $job_id == 146 || $job_id == 999 || $job_id == 0){
