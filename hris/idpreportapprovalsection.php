<?php
//--------------------------------------------------------------------//
// Filename : modules/hris/idpreportapprovalsection.php               //
...
if ( !defined('HRIS_IDPREPORTSECTIONAPPROVAL_DEFINED') ) {
   define('HRIS_IDPREPORTSECTIONAPPROVAL_DEFINED', TRUE);
...
class _hris_IDPReportApprovalSection extends XocpBlock {
   var $catchvar = _HRIS_CATCH_VAR;
   var $blockID = _HRIS_IDPREPORTSECTIONAPPROVAL_BLOCK;
   var $width = "100%";
   var $language;
   var $display_title = TRUE;
   var $title = _HRIS_IDPREPORTSECTIONAPPROVAL_BLOCK_TITLE;
   var $display_comment = TRUE;
   var $data;
   
   function _hris_IDPReportApprovalSection($catch=NULL) { /* fungsi konstruktor wajib punya parameter $catch
   ...
   function approval_project($request_id,$project_id) {
      $db=&Database::getInstance();
      $_SESSION["html"]->js_tinymce = TRUE;
      $user_id = getUserID();
      list($self_job_id,
      ...
      if(1) {
         ...
         if($method_t!="") {
            ...
            if(file_exists($editor_file)) {
               require_once($editor_file);
               $freport = "${method_t}_idp_m_getReportingForm";
               $form = $freport($request_id,$project_id);
               $ret = "<div style='text-align:right;border:1px solid #bbb;background-color:#ddd;padding:3px;'>"
                    . "[<a href='".XOCP_SERVER_SUBDIR."/index.php'>back</a>]"
                    . "</div><div style='border:0px solid #ddd;border-top:0;padding:3px;padding-top:10px;'>$form</div>";
            }
   ...
   function requestList() {
      ...
      $sql = "SELECT e.event_title,a.employee_id,a.request_id,a.actionplan_id,a.event_id,b.method_t,bt.method_type,d.person_nm,a.submit_dttm,a.status_cd"
           ...
           . " WHERE a.section_mgr_job_id = '$self_job_id'"
      ...
      if($db->getRowsNum($result)>0) {
         while(list($event_title,$employee_id,$request_id,$actionplan_id,$event_id,$method_t,$method_type,$employee_nm,$submit_dttm,$status_cd)=$db->fetchRow($result)) {
            if($status_cd=="approval1") {
               ...
               $link_txt = "Next Superior APproval";
      ...
      $sql = "SELECT a.book_title,a.employee_id,a.request_id,a.actionplan_id,b.method_t,bt.method_type,d.person_nm,a.submit_dttm,a.status_cd"
           . " FROM ".XOCP_PREFIX."idp_report_SELF_1 a"
           . " LEFT JOIN ".XOCP_PREFIX."idp_request_actionplan b USING(request_id,actionplan_id)"
           . " LEFT JOIN ".XOCP_PREFIX."idp_development_method_type bt USING(method_t)"
           . " LEFT JOIN ".XOCP_PREFIX."employee c ON c.employee_id = a.employee_id"
           . " LEFT JOIN ".XOCP_PREFIX."persons d USING(person_id)"
           . " WHERE a.approval1_job_id = '$self_job_id'"
           . " AND a.status_cd IN ('approval1','approval2','completed')";
      $result = $db->query($sql);
      _debuglog($sql);
      if($db->getRowsNum($result)>0) {
         while(list($event_title,$employee_id,$request_id,$actionplan_id,$method_t,$method_type,$employee_nm,$submit_dttm,$status_cd)=$db->fetchRow($result)) {
            if($status_cd=="approval1") {
               $link_txt = "Click to Approve";
            } else if($status_cd=="approval2") {
               $link_txt = "DM Approval";
      ...
      $sql = "SELECT a.request_id,a.project_id,d.person_nm,a.project_nm,a.report_submit_dttm,a.report_status_cd"
           ...
           . " LEFT JOIN ".XOCP_PREFIX."persons d USING(person_id)"
           . " WHERE a.report_section_mgr_job_id = '$self_job_id'"
           . " AND a.report_status_cd IN ('prepared','approval1','approval2','completed')";
      $result = $db->query($sql);
      if($db->getRowsNum($result)>0) {
         while(list($request_id,$project_id,$employee_nm,$project_nm,$submit_dttm,$status_cd)=$db->fetchRow($result)) {
            if($status_cd=="approval1") {
               $link_txt = "Click to Approve";
            } else if($status_cd=="approval2") {
               $link_txt = "DM Approval";
            ...
            $link = "<a href='".XOCP_SERVER_SUBDIR."/index.php?".$this->getURLParam()."&approve_project=y&request_id=${request_id}&project_id=${project_id}'>$link_txt</a>&nbsp;";
      ...
      switch ($this->catch) {
         case $this->blockID:
            if(isset($_GET["smethod"])&&$_GET["smethod"]==1&&isset($_GET["t"])&&$_GET["t"]!="") {
             ...
             } else if(isset($_GET["approve_project"])&&$_GET["approve_project"]=="y") {
               $request_id = $_GET["request_id"];
               $project_id = $_GET["project_id"];
               $ret = $this->approval_project($request_id,$project_id);
...
} // HRIS_IDPREPORTSECTIONAPPROVAL_DEFINED
?>
