...
function COACH_idp_m_getReportingEventForm($event_id,$employee_id) {
...
function COACH_idp_m_getReportingForm($request_id,$actionplan_id) {
   require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
   require_once(XOCP_DOC_ROOT."/modules/hris/include/idp/ajax/ajax_method_COACH.php");
   $ajax = new _idp_class_method_COACH_ajax("mCOACH");
   $db=&Database::getInstance();
   $user_id = getUserID();
   ...
   $sql = "SELECT subject_txt,r_01,r_02,r_03,r_04,r_05,r_06,r_07,"
        ...
        . " FROM ".XOCP_PREFIX."idp_report_COACH"
   ...
   if($db->getRowsNum($result)==1) {
      ...
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_COACH (employee_id,actionplan_id,request_id,subject_txt,approval1_job_id,approval1_emp_id,approval2_job_id,approval2_emp_id)"
   ...
   $form = "<div style='text-align:center;font-size:1.2em;font-weight:bold;'>COACHING REPORT</div>"
         ...
         . "<div style='padding:10px;'>"
            . "<div style='font-weight:bold;'>A. Coaching Summary:</div>"
   ...
   $js = $ajax->getJs()."<script type='text/javascript'><!--
      ...
      function confirm_delete_subject(subject_id,d,e) {
         ...
         _destroy(tr);
         mCOACH_app_deleteSubject('$request_id','$actionplan_id',subject_id,null);
      }
      
      function new_subject() {
         $('subjectprogress').innerHTML = '';
         $('subjectprogress').appendChild(progress_span());
         mCOACH_app_newSubject('$request_id','$actionplan_id',function(_data) {
            $('subjectprogress').innerHTML = '';
      ...
      function confirm_delete_idea(subject_id,idea_id,d,e) {
         ...
         _destroy(tr);
         mCOACH_app_deleteIdea('$request_id','$actionplan_id',subject_id,idea_id,null);
      }
      
      function new_idea(subject_id) {
         $('ideaprogress_'+subject_id).innerHTML = '';
         $('ideaprogress_'+subject_id).appendChild(progress_span());
         mCOACH_app_newIdea('$request_id','$actionplan_id',subject_id,function(_data) {
            var data = recjsarray(_data);
      ...
      function confirm_delete_idea(idea_id,d,e) {
         ...
         _destroy(tr);
         mCOACH_app_deleteIdea('$event_id','$employee_id',idea_id,null);
      }
      
      function new_idea() {
         $('ideaprogress').innerHTML = '';
         $('ideaprogress').appendChild(progress_span());
         mCOACH_app_newIdea('$event_id','$employee_id',function(_data) {
            $('ideaprogress').innerHTML = '';
            var data = recjsarray(_data);
            var tr = _dce('tr');
            tr.td0 = tr.appendChild(_dce('td'));
      ...
      */
      ...
      function do_approval2() {
         ...
         mCOACH_app_approval2Report('$request_id','$actionplan_id',function(_data) {
      ...
      function do_return1() {
         ...
         mCOACH_app_return1Report(\"$request_id\",\"$actionplan_id\",return_note,function(_data) {
            var data = recjsarray(_data);
            location.href = '".XOCP_SERVER_SUBDIR."/index.php?approve=y&request_id=${request_id}&actionplan_id=${actionplan_id}&r='+uniqid('r');
         });
      }
      ...
      function do_approval1() {
         $('confirmbtn').innerHTML = '';
         $('confirmbtn').appendChild(progress_span(' ... approve report'));
         mCOACH_app_approval1Report('$request_id','$actionplan_id',function(_data) {
      ...
      function do_submit() {
         ...
         mCOACH_app_submitReport('$request_id','$actionplan_id',function(_data) {
      ...
      function reedit_subject(subject_id,d,e) {
         ...
         mCOACH_app_reeditSubject('$request_id','$actionplan_id',subject_id,function(_data) {
            var data = recjsarray(_data);
      ...
      function save_subject(subject_id,read_only,d,e) {
         ...
         mCOACH_app_saveReportSubject('$request_id','$actionplan_id',subject_id,urlencode(subject_txt),idea,read_only,subject_dttm,function(_data) {
            var data = recjsarray(_data);
            if(data[2]==0) {
               setTimeout(\"$('progress_subject_\"+data[0]+\"').innerHTML = '';\",1000);
            } else {
      ...
      function save_report(d,e) {
         ...
         mCOACH_app_saveReport('$request_id','$actionplan_id',retsubject,rating,idea,function(_data) {
...
function COACH_idp_m_getRemark($request_id,$actionplan_id) {
   $db=&Database::getInstance();
   ...
   if($db->getRowsNum($result)==1) {
      ...
      $sql = "SELECT status_cd FROM ".XOCP_PREFIX."idp_report_COACH"
...
function COACH_idp_m_editActionPlan($request_id,$actionplan_id) {
   $db=&Database::getInstance();
...
function COACH_idp_m_saveAction($request_id,$actionplan_id,$ret) {
   $db=&Database::getInstance();
   $vars = _parseForm($ret);
...
function _idpm_save(request_id,actionplan_id) {
   ...
   save_ap('COACH',request_id,actionplan_id,ret);
}

function _COACH_doselecttemplate(selfstudy_id,selfstudy_nm,d,e) {
...
function _COACH_doselectother(d,e) {
...
function _COACH_selectselfstudy(request_id,actionplan_id,d,e) {
