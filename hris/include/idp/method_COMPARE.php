...
function COMPARE_idp_m_getReportingEventForm($event_id,$employee_id) {
...
function COMPARE_idp_m_getReportingForm($request_id,$actionplan_id) {
   require_once(XOCP_DOC_ROOT."/modules/hris/include/hris.php");
   require_once(XOCP_DOC_ROOT."/modules/hris/include/idp/ajax/ajax_method_COMPARE.php");
   $ajax = new _idp_class_method_COMPARE_ajax("mCOMPARE");
   $db=&Database::getInstance();
   $user_id = getUserID();
   ...
   $sql = "SELECT company_nm,company_profile,industry,comparative_item,finding,"
        . "submit_dttm,"
        ...
        . " FROM ".XOCP_PREFIX."idp_report_COMPARE"
        . " WHERE request_id = '$request_id'"
        . " AND actionplan_id = '$actionplan_id'";
   $result = $db->query($sql);
   _debuglog($sql);
   if($db->getRowsNum($result)==1) {
      list($company_nm,$company_profile,$industry,$comparative_item,$finding,
      ...
      $company_profile = htmlentities($company_profile,ENT_QUOTES);
      $industry = htmlentities($industry,ENT_QUOTES);
      ...
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_COMPARE (employee_id,actionplan_id,request_id,company_nm,approval1_job_id,approval1_emp_id,approval2_job_id,approval2_emp_id)"
   ...
   $form = "<div style='text-align:center;font-size:1.2em;font-weight:bold;'>COMPARATIVE STUDY REPORT</div>"
         
         . "<div style='margin-bottom:10px;margin-top:10px;padding:5px;border:1px solid #bbb;-moz-border-radius:5px;'>"
            . "<table style='width:100%;'><tbody>"
            . "<tr><td style='text-align:right;font-weight:bold;'>Company Name : </td><td>$method_subject</td></tr>"
            . "<tr><td style='text-align:right;font-weight:bold;'>Time frame : </td><td>".sql2ind($plan_start_dttm,"date")." - ".sql2ind($plan_stop_dttm,"date")."</td></tr>"
         ...
         . "<div style='padding:10px;'>"
            . "<div style='font-weight:bold;'>A. Comparative Study Summary:</div>"
            . "<div style='padding:10px;padding-left:15px;' id='dvsummary'>"
               . "<table class='xxlist' style='width:100%;'><colgroup><col/></colgroup>"
               . "<thead>"
               . ($preview==0&&$status_cd=="prepared"?"<tr><td colspan='2' style='text-align:right;'>&nbsp;</td></tr>":"")
               //. "<tr><td style='text-align:left;'>Summary</td></tr>"
               . "</thead>"
               . "<tbody id='tbodysubject'>"
               . "<tr><td style='text-align:right;'>Company Name :</td><td>$method_subject</td></tr>"
               . "<tr><td style='text-align:right;'>Company Profile :</td><td>".($status_cd=="prepared"?"<input value='$company_profile' id='company_profile' name='company_profile' type='text' style='width:300px;'/>":$company_profile)."</td></tr>"
               . "<tr><td style='text-align:right;'>Industry :</td><td>".($status_cd=="prepared"?"<input value='$industry' id='industry' name='industry' type='text' style='width:300px;'/>":$industry)."</td></tr>"
               . "<tr><td style='text-align:right;'>Comparative Item :</td><td><div style='width:450px;' id='comparative_item'>$comparative_item</div></td></tr>"
               . "<tr><td style='text-align:right;'>Finding :</td><td><div style='width:450px;' id='finding'>$finding</div></td></tr>"
               . "</tbody>"
               . "</table>"
            . "</div>"
         . "</div>"
         ...
         . "<div style='padding:10px;'>"
            . "<div style='font-weight:bold;'>B. Implementation in Organization:</div>"
   ...
   $js = $ajax->getJs()."<script type='text/javascript'><!--
      ...
      function confirm_delete_subject(subject_id,d,e) {
         ...
         _destroy(tr);
         mCOMPARE_app_deleteSubject('$request_id','$actionplan_id',subject_id,null);
      }
      
      function new_subject() {
         $('subjectprogress').innerHTML = '';
         $('subjectprogress').appendChild(progress_span());
         mCOMPARE_app_newSubject('$request_id','$actionplan_id',function(_data) {
            $('subjectprogress').innerHTML = '';
      ...
      function confirm_delete_idea(idea_id,d,e) {
         ...
         _destroy(tr);
         mCOMPARE_app_deleteIdea('$request_id','$actionplan_id',idea_id,null);
      }
      
      function new_idea() {
         $('ideaprogress').innerHTML = '';
         $('ideaprogress').appendChild(progress_span());
         mCOMPARE_app_newIdea('$request_id','$actionplan_id',function(_data) {
            $('ideaprogress').innerHTML = '';
            var data = recjsarray(_data);
            var tr = _dce('tr');
            tr.td0 = tr.appendChild(_dce('td'));
      ...
      function confirm_delete_idea(idea_id,d,e) {
         var tr = $('tridea_'+idea_id);
         _destroy(tr);
         mCOMPARE_app_deleteIdea('$event_id','$employee_id',idea_id,null);
      }
      
      function new_idea() {
         $('ideaprogress').innerHTML = '';
         $('ideaprogress').appendChild(progress_span());
         mCOMPARE_app_newIdea('$event_id','$employee_id',function(_data) {
            $('ideaprogress').innerHTML = '';
            var data = recjsarray(_data);
            var tr = _dce('tr');
            tr.td0 = tr.appendChild(_dce('td'));
      ...
      */
      ...
      function do_approval2() {
         ...
         mCOMPARE_app_approval2Report('$request_id','$actionplan_id',function(_data) {
      ...
      function do_return1() {
         ...
         mCOMPARE_app_return1Report(\"$request_id\",\"$actionplan_id\",return_note,function(_data) {
            var data = recjsarray(_data);
            location.href = '".XOCP_SERVER_SUBDIR."/index.php?approve=y&request_id=${request_id}&actionplan_id=${actionplan_id}&r='+uniqid('r');
         });
      }
      ...
      function do_approval1() {
         $('confirmbtn').innerHTML = '';
         $('confirmbtn').appendChild(progress_span(' ... approve report'));
         mCOMPARE_app_approval1Report('$request_id','$actionplan_id',function(_data) {
      ...
      function do_submit() {
         ...
         mCOMPARE_app_submitReport('$request_id','$actionplan_id',function(_data) {
      ...
      function reedit_subject(subject_id,d,e) {
         ...
         mCOMPARE_app_reeditSubject('$request_id','$actionplan_id',subject_id,function(_data) {
            var data = recjsarray(_data);
      ...
      function save_subject(subject_id,read_only,d,e) {
         ...
         mCOMPARE_app_saveReportSubject('$request_id','$actionplan_id',subject_id,urlencode(subject_txt),idea,read_only,subject_dttm,function(_data) {
            var data = recjsarray(_data);
            if(data[2]==0) {
               setTimeout(\"$('progress_subject_\"+data[0]+\"').innerHTML = '';\",1000);
            } else {
      ...
      function save_report(d,e) {
         ...
         var retsubject = '';
         var company_profile = urlencode($('company_profile').value);
         var industry = urlencode($('industry').value);
         var comparative_item = tinyMCE.get('comparative_item').getContent();
         comparative_item = urlencode(comparative_item);
         var finding = tinyMCE.get('finding').getContent();
         finding = urlencode(finding);
         $('progress').innerHTML = '';
         $('progress'). appendChild(progress_span(' saving ... '));
         mCOMPARE_app_saveReport('$request_id','$actionplan_id',company_profile,industry,comparative_item,finding,idea,function(_data) {
      ...
      function init_my_tiny() {
         if(1) {
            tinyMCE.init({
               mode : 'exact',
               elements : 'comparative_item,finding',
               theme : 'advanced',
               theme_advanced_buttons1 : 'bold,italic,underline,strikethrough,separator,bullist,numlist,outdent,indent,separator,justifyleft,justifycenter,justifyright,separator,forecolor,cut,copy,pasteword',
...
function COMPARE_idp_m_getRemark($request_id,$actionplan_id) {
   $db=&Database::getInstance();
   ...
   if($db->getRowsNum($result)==1) {
      ...
      $sql = "SELECT status_cd FROM ".XOCP_PREFIX."idp_report_COMPARE"
...
function COMPARE_idp_m_editActionPlan($request_id,$actionplan_id) {
   $db=&Database::getInstance();
   ...
   $ret = "<table id='method_coach_${request_id}_${actionplan_id}' style='width:100%;'>"
        ...
        . "<tr><td style='text-align:right;'>Company Name : </td><td class='cb' colspan='3' style='vertical-align:middle;'>"
...
function COMPARE_idp_m_saveAction($request_id,$actionplan_id,$ret) {
   $db=&Database::getInstance();
   $vars = _parseForm($ret);
   $sql = "UPDATE ".XOCP_PREFIX."idp_request_actionplan SET "
        ...
        . "cost_estimate = '"._bctrim(bcadd($vars["cost_estimate"],0))."'"
        . " WHERE request_id = '$request_id'"
        . " AND actionplan_id = '$actionplan_id'";
   $db->query($sql);
}
...
function _idpm_save(request_id,actionplan_id) {
   ...
   save_ap('COMPARE',request_id,actionplan_id,ret);
}

function _COMPARE_doselecttemplate(selfstudy_id,selfstudy_nm,d,e) {
...
function _COMPARE_doselectother(d,e) {
...
function _COMPARE_selectselfstudy(request_id,actionplan_id,d,e) {
