<?php
//--------------------------------------------------------------------//
// Filename : modules/hris/include/idp/ajax/ajax_method_TRN_EX.php   //
...
if ( !defined('HRIS_IDPMETHODTRN_EXAJAX_DEFINED') ) {
   define('HRIS_IDPMETHODTRN_EXAJAX_DEFINED', TRUE);
...
class _idp_class_method_TRN_EX_ajax extends AjaxListener {
   
   function _idp_class_method_TRN_EX_ajax($act_name) {
      $this->_act_name = $act_name;
      $this->_include_file = XOCP_DOC_ROOT."/modules/hris/include/idp/ajax/ajax_method_TRN_EX.php";
      $this->init();
      parent::init();
   }
   ...
   function app_divisionManagerReturnReport($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX SET status_cd = 'approval1', return_note = '$return_note'"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $db->query($sql);
      $sql = "SELECT section_mgr_emp_id,request_id,actionplan_id FROM ".XOCP_PREFIX."idp_report_TRN_EX"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $result = $db->query($sql);
   ...
   function app_sectionManagerReturnReport($args) {
      ...
      $employee_id = $args[1];
      $return_note = addslashes(trim(urldecode($args[2])));
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX SET status_cd = 'prepared', return_note = '$return_note'"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $db->query($sql);
      $sql = "SELECT request_id,actionplan_id FROM ".XOCP_PREFIX."idp_report_TRN_EX"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $result = $db->query($sql);
   ...
   function app_deleteIdea($args) {
      ...
      $employee_id = $args[1];
      $idea_id = $args[2];
      $sql = "DELETE FROM ".XOCP_PREFIX."idp_report_TRN_EX_ideas"
   ...
   function app_newIdea($args) {
      ...
      $event_id = $args[0];
      $employee_id = $args[1];
      $sql = "SELECT MAX(idea_id) FROM ".XOCP_PREFIX."idp_report_TRN_EX_ideas WHERE event_id = '$event_id' AND employee_id = '$employee_id'";
      $result = $db->query($sql);
      $idea_id = 0;
      if($db->getRowsNum($result)>0) {
      ...
      $idea_id++;
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_TRN_EX_ideas (event_id,employee_id,idea_id) VALUES ('$event_id','$employee_id','$idea_id')";
      $db->query($sql);
   ...
   function app_approval2Report($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX SET status_cd = 'completed', division_mgr_approve_dttm = now()"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $db->query($sql);
   ...
   function app_approval1Report($args) {
      ...
      $sql = "SELECT request_id,actionplan_id,division_mgr_emp_id FROM ".XOCP_PREFIX."idp_report_TRN_EX"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $result = $db->query($sql);
      ...
      if($doubleapproval==1) {
         $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX SET status_cd = 'approval2', section_mgr_approve_dttm = now(), return_note = ''"
              . " WHERE event_id = '$event_id'"
              . " AND employee_id = '$employee_id'";
         $db->query($sql);
         ...
         $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX SET status_cd = 'completed', section_mgr_approve_dttm = now(), return_note = ''"
              . " WHERE event_id = '$event_id'"
              . " AND employee_id = '$employee_id'";
         $db->query($sql);
   ...
   function app_submitReport($args) {
      ...
      $event_id = $args[0];
      $employee_id = $args[1];
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX SET return_note = '', status_cd = 'approval1', submit_dttm = now()"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $db->query($sql);
      $sql = "SELECT request_id,actionplan_id,section_mgr_emp_id FROM ".XOCP_PREFIX."idp_report_TRN_EX"
           . " WHERE event_id = '$event_id'"
           . " AND employee_id = '$employee_id'";
      $result = $db->query($sql);
   ...
   function app_saveReport($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX SET "
      ...
      if(isset($varsidea)&&count($varsidea)>0) {
         $sql = "SELECT idea_id FROM ".XOCP_PREFIX."idp_report_TRN_EX_ideas WHERE event_id = '$event_id' AND employee_id = '$employee_id'";
         $result = $db->query($sql);
         if($db->getRowsNum($result)>0) {
            while(list($idea_id)=$db->fetchRow($result)) {
               ...
               $sql = "UPDATE ".XOCP_PREFIX."idp_report_TRN_EX_ideas SET "
                    . "idea_txt = '$text',"
...
} /// HRIS_IDPMETHODTRN_EXAJAX_DEFINED
?>