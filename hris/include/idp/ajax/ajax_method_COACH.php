<?php
//--------------------------------------------------------------------//
// Filename : modules/hris/include/idp/ajax/ajax_method_COACH.php     //
...
if ( !defined('HRIS_IDPMETHODCOACHAJAX_DEFINED') ) {
   define('HRIS_IDPMETHODCOACHAJAX_DEFINED', TRUE);
...
class _idp_class_method_COACH_ajax extends AjaxListener {
   
   function _idp_class_method_COACH_ajax($act_name) {
      $this->_act_name = $act_name;
      $this->_include_file = XOCP_DOC_ROOT."/modules/hris/include/idp/ajax/ajax_method_COACH.php";
      $this->init();
      parent::init();
   }
   ...
   function app_reeditSubject($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH_subject SET "
           . "read_only = '0'"
   ...
   function renderSubjectEdit($request_id,$actionplan_id,$subject_id) {
      ...
      $sql = "SELECT subject_txt,subject_dttm"
           . " FROM ".XOCP_PREFIX."idp_report_COACH_subject"
      ...
      if($db->getRowsNum($result)>0) {
         ...
         if(1) {
            ...
            $sql = "SELECT idea_id,idea_txt,start_dttm,stop_dttm"
                 . " FROM ".XOCP_PREFIX."idp_report_COACH_ideas"
                 . " WHERE request_id = '$request_id'"
                 . " AND actionplan_id = '$actionplan_id'"
                 . " AND subject_id = '$subject_id'"
                 . " ORDER BY idea_id";
   ...
   function app_saveReportSubject($args) {
      ...
      if(isset($idea)&&count($idea)>0) {
         $sql = "SELECT idea_id FROM ".XOCP_PREFIX."idp_report_COACH_ideas WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
         $result = $db->query($sql);
         if($db->getRowsNum($result)>0) {
            while(list($idea_id)=$db->fetchRow($result)) {
               ...
               $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH_ideas SET "
                    . "idea_txt = '$text',"
   ...
   function app_deleteSubject($args) {
      ...
      $actionplan_id = $args[1];
      $subject_id = $args[2];
      $sql = "DELETE FROM ".XOCP_PREFIX."idp_report_COACH_subject"
           . " WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'"
           . " and subject_id = '$subject_id'";
      $db->query($sql);
      $sql = "DELETE FROM ".XOCP_PREFIX."idp_report_COACH_ideas"
           . " WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'"
           . " and subject_id = '$subject_id'";
      $db->query($sql);
   }
   ...
   function app_newSubject($args) {
      ...
      $request_id = $args[0];
      $actionplan_id = $args[1];
      $sql = "SELECT MAX(subject_id) FROM ".XOCP_PREFIX."idp_report_COACH_subject WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
      $subject_id = 0;
      if($db->getRowsNum($result)>0) {
      ...
      $subject_id++;
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_COACH_subject (request_id,actionplan_id,subject_id,subject_dttm) VALUES ('$request_id','$actionplan_id','$subject_id',now())";
      $db->query($sql);
      /*
   ...
   function app_divisionManagerReturnReport($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH SET status_cd = 'approval1', return_note = '$return_note'"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
      $sql = "SELECT approval1_emp_id,request_id,actionplan_id FROM ".XOCP_PREFIX."idp_report_COACH"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
   ...
   function app_return1Report($args) {
      ...
      $actionplan_id = $args[1];
      $return_note = addslashes(trim(urldecode($args[2])));
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH SET status_cd = 'prepared', return_note = '$return_note'"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
   ...
   function app_newIdea($args) {
      ...
      $actionplan_id = $args[1];
      $subject_id = $args[2];
      $sql = "SELECT MAX(idea_id) FROM ".XOCP_PREFIX."idp_report_COACH_ideas WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id' AND subject_id = '$subject_id'";
      $result = $db->query($sql);
      $idea_id = 0;
      if($db->getRowsNum($result)>0) {
      ...
      $idea_id++;
      $sql = "INSERT INTO ".XOCP_PREFIX."idp_report_COACH_ideas (request_id,actionplan_id,subject_id,idea_id) VALUES ('$request_id','$actionplan_id','$subject_id','$idea_id')";
      $db->query($sql);
   ...
   function app_approval2Report($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH SET status_cd = 'completed', approval2_dttm = now()"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
      
      $sql = "SELECT request_id,actionplan_id FROM ".XOCP_PREFIX."idp_report_COACH"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
   ...
   function app_approval1Report($args) {
      ...
      $sql = "SELECT approval2_emp_id FROM ".XOCP_PREFIX."idp_report_COACH"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
      ...
      if($doubleapproval==1) {
         $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH SET status_cd = 'approval2', approval1_dttm = now(), return_note = ''"
              . " WHERE request_id = '$request_id'"
              . " AND actionplan_id = '$actionplan_id'";
         $db->query($sql);
         ...
         $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH SET status_cd = 'completed', approval1_dttm = now(), return_note = ''"
              . " WHERE request_id = '$request_id'"
              . " AND actionplan_id = '$actionplan_id'";
         $db->query($sql);
   ...
   function app_submitReport($args) {
      ...
      $request_id = $args[0];
      $actionplan_id = $args[1];
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH SET return_note = '', status_cd = 'approval1', submit_dttm = now()"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $db->query($sql);
      $sql = "SELECT approval1_emp_id FROM ".XOCP_PREFIX."idp_report_COACH"
           . " WHERE request_id = '$request_id'"
           . " AND actionplan_id = '$actionplan_id'";
      $result = $db->query($sql);
   ...
   function app_saveReport($args) {
      ...
      $sql = "UPDATE ".XOCP_PREFIX."idp_report_COACH SET "
           . "r_01 = '$r_01',"
           . "r_02 = '$r_02',"
      ...
      if(isset($idea)&&count($idea)>0) {
         $sql = "SELECT subject_id,idea_id FROM ".XOCP_PREFIX."idp_report_COACH_ideas WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
         $result = $db->query($sql);
         if($db->getRowsNum($result)>0) {
      ...
      if(isset($subject)&&count($subject)>0) {
         $sql = "SELECT subject_id FROM ".XOCP_PREFIX."idp_report_COACH_subject WHERE request_id = '$request_id' AND actionplan_id = '$actionplan_id'";
         $result = $db->query($sql);
         if($db->getRowsNum($result)>0) {
...
} /// HRIS_IDPMETHODCOACHAJAX_DEFINED
?>