...
require_once(RELATIVE_PATH.'fpdf_xhtml.php');
//require_once(RELATIVE_PATH.'fpdf_protection.php');
  ...
  //Explode the string in order to parse the HTML code
	...
	foreach($a as $i => $e)
	{
		...
		if($i%2==0)
		{
			...
			//Adjust lineheight
      //			$this->lineheight = (4*$this->FontSizePt)/11; //should be inside printbuffer?
...
//! @return void
  ...
  if($tag=='OUTLINE')
  {
	 ...
	 if(!$this->tablestart)
   {
    ...
    $this->textbuffer=array();
  	if ($tag=='P') $this->Ln(0.6*$this->lineheight);
...
//! @desc Prints all list-related buffered info
    ...
    foreach($this->listitem as $item)
    {
        ...
        if ($type == chr(149)) $this->SetFont('arial','',14);
        if ($type == chr(186)) $this->SetFont('arial','',14);
        $type .= ' ';
        $blt_width = $this->GetStringWidth($type)+$this->cMargin*2;
        //Output bullet
        $this->Cell($blt_width,4,$type,'','','L');
			  ...
			  case 'FONT-SIZE': //Does not support: smaller, larger
			      if(is_numeric($v{0}))
			      {
  			      ...
  			      switch($v)
  			      {
  			         ...
  			         case 'XX-SMALL': $this->SetFontSize( (0.7)* 9);
  			             break;
                 case 'X-SMALL': $this->SetFontSize( (0.77) * 9);
			               break;
			           case 'SMALL': $this->SetFontSize( (0.86)* 9);
  			             break;
  			         case 'MEDIUM': $this->SetFontSize(9);
  			             break;
  			         case 'LARGE': $this->SetFontSize( (1.2)*9);
  			             break;
  			         case 'X-LARGE': $this->SetFontSize( (1.5)*9);
  			             break;
  			         case 'XX-LARGE': $this->SetFontSize( 2*9);
			               break;
              }
            }
			   	  break;
